...
export default function ProductDetail({
  ...
  const pieceQty = qty;
  /**
   * 🎨 เพดานช่อง "คละกี่ลาย" ของงานที่หลายลายอยู่บนแผ่นเดียวกันได้ (กติกาคละไม่ได้บังคับ 1 ลาย/หน่วย)
   * กติกาเองไม่มีเพดาน (Infinity) แต่ปุ่ม +/− กับช่องพิมพ์ต้องมีตัวเลขจับต้องได้
   * = จำนวนชิ้นที่ตัดได้จริงทั้งออเดอร์ — สั่งโปสการ์ด 125 แผ่น ตัดได้แผ่นละ 8 ใบ → คละได้ 1,000 ลาย
   * ⚠️ เดิมตั้งตายไว้ 99 ลาย งานตัดแบ่งหลักพันใบเลยเลือกได้แค่ 99 (พนักงานแจ้ง 21 ก.ย. 69)
   * ยังอย่างน้อย 99 เสมอ — สินค้าที่ไม่ได้ตั้ง "1 หน่วยได้กี่ชิ้น" ไว้ต้องคละได้ไม่น้อยกว่าเดิม
   * (unitYield.unit ไม่ใช่ null = ตัวเลขเป็น "ต่อแผ่นวัสดุ" คนละหน่วยกับที่สั่ง คูณตรง ๆ ไม่ได้)
   */
  const freeMixCap = Math.max(
    99,
    unitYield && unitYield.unit == null && unitYield.per > 0 ? unitYield.per * Math.max(1, qty) : qty
  );
  ...
  const maxDesignsRaw = mixRule
    ? // ช่วงที่ยังไม่ถึงเกณฑ์ "1 ลาย/หน่วย" คละได้ไม่จำกัด (หลายลายอยู่บนแผ่นเดียวกันได้)
      // = Infinity → ช่อง +/− ต้องมีเพดานที่จับต้องได้ ใช้ "จำนวนชิ้นที่ได้จริงทั้งออเดอร์" (freeMixCap)
      // ส่วนช่วงที่บังคับ 1 ลาย/หน่วย เพดาน = จำนวนที่สั่ง ซึ่งเป็นเลขจริง ห้ามเอาเพดานสำรองไปกดทับ
      // (เคยพลาด: สั่ง 100 เซ็ต ควรคละได้ 100 ลาย แต่โดนตัดเหลือ 99)
      Math.max(1, finiteOr(mixMaxDesigns(mixRule, pieceQty), freeMixCap))
