 ...
 */
const isTailLine = (k: string) => /^เรทราคา|จำนวนลาย/.test(bareLabel(k));
...
/**
 * ♻️ "ใช้ไฟล์เก่า (ลายเดียวกับที่เคยสั่ง)" — ไปอยู่บรรทัดล่างสุดของทุกรายการ (เจ้าของร้านสั่ง 15 ก.ย. 69)
 * เป็นเรื่อง "เอาลายมาจากไหน" ไม่ใช่สเปคของชิ้นงาน — และมีป้ายของตัวเองอยู่ข้างชื่อรายการแล้ว
 */
const isReuseLine = (k: string) => /ไฟล์เก่า|ไฟล์เดิม/.test(bareLabel(k));
 ...
 *   → 5 เรทราคา/จำนวนลาย → 6 ใช้ไฟล์เก่า (ล่างสุด)
...
function specRank(k: string): number {
  ...
  if (isTailLine(k)) return 5;
  if (isBaseLine(k)) return isSizeLine(k) ? 3 : 4;
  if (isSizeLine(k)) return 0;
 ...
 *   • เรียงเป็นกลุ่มตาม specRank — ขนาดตัว + วัสดุ/สี + ข้อมูลตัว · ขนาดฐาน + ข้อมูลฐาน · เรทราคา/จำนวนลาย · ใช้ไฟล์เก่าล่างสุด
...
export function tidySpec(entries: [string, string][]): [string, string][] {
  const kept = entries.filter(([k, v]) => !pickedNone(k, v));
