...
/**
 * 🎞 บรรทัดฟอยล์ที่เลือกว่า "ไม่ทำ" — ไม่ต้องขึ้นบรรทัด (เจ้าของร้านสั่ง 15 ก.ย. 69)
 * งานส่วนใหญ่ไม่ปั๊มฟอยล์ บรรทัด "เคลือบฟอยล์: ไม่เคลือบฟอยล์" จึงเป็นแค่บรรทัดรก
 * สิ่งที่ทุกจอต้องเห็นคือ "ปั๊มฟอยล์สีอะไร" เท่านั้น — ไม่มีบรรทัด = ไม่มีฟอยล์
 */
const foilNone = (k: string, v: string) => /ฟอยล์/.test(k) && /^ไม่/.test(v.trim());
...
/** หัวข้อที่ไปอยู่ล่างสุด — เป็นข้อมูล "การสั่ง" ไม่ใช่สเปคของชิ้นงาน (เจ้าของร้านสั่ง 15 ก.ย. 69) */
const isTailLine = (k: string) => /เรทราคา|จำนวนลาย/.test(k);

/**
 * ลำดับกลุ่มของหัวข้อ — เรียงจากชิ้นงานหลักลงไปหาข้อมูลการสั่ง
 *   0 ขนาดของตัวงาน → 1 รายละเอียดของตัวงาน → 2 ขนาดฐาน → 3 รายละเอียดฐาน → 4 เรทราคา/จำนวนลาย
...
function specRank(k: string): number {
  if (isTailLine(k)) return 4;
  if (isBaseLine(k)) return isSizeLine(k) ? 2 : 3;
  return isSizeLine(k) ? 0 : 1;
}
 ...
 *
 *   • ฟอยล์ที่เลือก "ไม่ทำ" = ตัดทิ้ง (foilNone)
 ...
 *   • เรียงเป็นกลุ่มตาม specRank — ขนาดตัว + ข้อมูลตัว · ขนาดฐาน + ข้อมูลฐาน · แล้วค่อยเรทราคา/จำนวนลาย
...
export function tidySpec(entries: [string, string][]): [string, string][] {
  const kept = entries.filter(([k, v]) => !foilNone(k, v));
