...
/**
 * 🕳 บรรทัดเจาะรู/เจาะตาไก่ — "ไม่เจาะรู" คือรูปทรงของชิ้นงานที่สั่ง ไม่ใช่ของเสริมที่ไม่ได้ทำ
 * ทีมผลิตต้องเห็นคำว่า "ไม่เจาะ" ไม่งั้นเจาะตามเคยแล้วงานเสีย (พนักงานแจ้ง 24 ก.ย. 69 · เฟรมการ์ดใส)
 * เช็คทั้งหัวข้อ ("เจาะรู: ไม่เจาะรู") และค่าที่เลือก — เฟรมการ์ดใสถามไว้ในกลุ่มชื่อ "ชิ้นงาน"
 */
const isHoleLine = (k: string, v: string) =>
  /เจาะ(รู|ตาไก่)/.test(bareLabel(k)) || /^ไม่(ต้อง)?เจาะ/.test(v.trim());
...
const pickedNone = (k: string, v: string) => {
  ...
  if (isSizeLine(k) || isCoatingLine(k) || isCutLine(k) || isHoleLine(k, t)) return false;
