...
export async function buildPrintBundle(input: {
  ...
  try {
    ...
    const jsx = [
      ...
      `    if (!tplFile.exists) { say(${th("เปิด TEMPLATE.ai ก่อน แล้วค่อยรันสคริปต์นี้")}); return; }`,
      "    app.open(tplFile);",
      "  }",
      "  var doc = app.activeDocument;",
      `  var step = ${th("สร้างเลเยอร์")};`,
      "  try {",
      "    var layer = doc.layers.add();",
      `    try { layer.name = ${th("ลายลูกค้า (iDucky)")}; } catch (e) { layer.name = "Customer Artwork (iDucky)"; }`,
      `    step = ${th("วางรูป (Place)")};`,
      "    var placed = layer.placedItems.add();",
      "    placed.file = artFile;",
      // ⚠️ ฝังรูปก่อนค่อยปรับขนาด — embed() บางเวอร์ชันสร้างอ็อบเจกต์ใหม่ทับ ทำให้ค่าที่ตั้งไว้ก่อนหน้าหาย
      `    step = ${th("ฝังรูป (Embed)")};`,
      "    try { placed.embed(); } catch (e) {}",
      `    step = ${th("ปรับขนาด/ตำแหน่ง")};`,
      "    var item = layer.pageItems.length ? layer.pageItems[0] : null;",
      `    if (!item) { say(${th("วางรูปไม่สำเร็จ — เลเยอร์ถูกสร้างแล้วแต่ยังว่าง")}); return; }`,
      "    var ab = doc.artboards[doc.artboards.getActiveArtboardIndex()].artboardRect;",
      "    item.width = ab[2] - ab[0];",
      "    item.height = ab[1] - ab[3];",
      "    item.position = [ab[0], ab[1]];",
      `    step = ${th("ย้ายเลเยอร์ลงล่างสุด")};`,
      "    layer.zOrder(ZOrderMethod.SENDTOBACK);",
      `    say(${th("เรียบร้อย — ลายอยู่ในเลเยอร์ ลายลูกค้า (iDucky) แล้ว")});`,
      "  } catch (e) {",
      `    say(${th("ติดที่ขั้นตอน: ")} + step + "\\n" + e);`,
      "  }",
      "})();",
      "",
    ].join("\n");
