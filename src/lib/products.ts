...
export interface ProductOptionChoice {
   ...
   * โชว์ในกลุ่ม display "cards" (ในการ์ด แบบเดียวกับ desc) และ "dropdown" (ใต้เมนู — ใส่ในรายการ <option> ไม่ได้)
   * เช่น "📄 ขนาดตามไฟล์" ในกลุ่มขนาดตัดของสติ๊กเกอร์/กระดาษ (11 ก.ย. 69)
   */
  selectedNote?: string;
  /**
  ...
  stockQtyPer?: number;
  /**
   * 📦 ของชิ้นอื่นที่ต้องหยิบเพิ่มเมื่อเลือกค่านี้ "และ" กลุ่มอื่นตรงเงื่อนไข (when ทุกข้อต้องจริง · ไม่มี when = ตัดเสมอ)
   * ใช้เมื่อของบนชั้นขึ้นกับ 2 ตัวเลือกพร้อมกัน — กรอบรูป+จิ๊กซอว์ งาน UV: ขนาด 15*20 ตัด "แผ่นจิ๊กซอว์ 15×20" เสมอ (stockItemId)
   * และตัด "กรอบรูป 15×20" เพิ่มเฉพาะเมื่อ ตัวเลือก = กรอบรูป + แผ่นจิ๊กซอว์ (stockLinks)
   * ตั้งจากปุ่ม "แยกตามตัวเลือก" หน้า /admin/stock · อ่านที่ loadOptionStockMap (lib/server/stock.ts)
   */
  ...
  /**
   * 🎨 สีตัวหนังสือของตัวเลือกนี้ในเมนูเลื่อน (CSS color เช่น "#15803d") — ให้ตัวเลือกบางกลุ่มโดดออกมาจากเพื่อน
   * เช่น ขนาดตัด "ครึ่ง A4/A5/A6 แนวตั้ง" เป็นสีเขียว (เจ้าของร้านสั่ง 9 ก.ย. 69) · ตั้งจากสคริปต์ หน้าแก้ไขแค่ส่งกลับ
   * ⚠️ มีผลเฉพาะ display "dropdown" บนเดสก์ท็อป — ตัวเลือกใน picker ของ iOS ทาสีไม่ได้ (ข้อจำกัดเบราว์เซอร์)
   */
  color?: string;
  /**
...
export interface ProductOption {
  ...
  chartSrc?: string;
  /**
   * 🖼 ภาพตัวอย่างประจำ "กลุ่มตัวเลือก" — โชว์เป็นภาพย่อข้างชื่อกลุ่มบนหน้าสินค้า
   * ใช้กับกลุ่มที่ชื่ออย่างเดียวนึกภาพไม่ออก โดยเฉพาะกลุ่มของเสริมที่ปิดไว้ก่อน (collapsible)
   * ที่ปิดอยู่เห็นแค่บรรทัดเดียว — ลูกค้าจะได้รู้ว่าของเสริมนี้หน้าตายังไงก่อนกดเปิด
   * ⚠️ รูปนี้ต้องถูกเติมเข้าแกลเลอรีด้วย (galleryImages) ไม่งั้นกดแล้วภาพใหญ่ไม่สลับตาม
   * กลุ่มของเสริม: ภาพอยู่ในแถวสวิตช์ กดทั้งแถว = เปิดกลุ่ม + สลับภาพใหญ่ไปที่ภาพนี้
   * (ซ้อนปุ่มกดรูปแยกในปุ่มแถวไม่ได้) · กลุ่มปกติ: กดที่ภาพ = สลับภาพใหญ่
   * คนละอันกับ noteImageSrc ที่เป็นรูปประกอบคำอธิบาย (ปุ่ม 👀 ท้าย note เปิดดูเต็มจอ)
   * ไม่มีผลกับราคา/ตะกร้า/ออเดอร์
   */
  imageSrc?: string;
  /**
...
export interface SheetYield {
   ...
   * ⚠️ คู่กับ longestOnly: ตารางคิดเหมือน "ชิ้นจัตุรัสด้านยาวสุด" → ใช้เฉพาะตอนรู้ด้านเดียว
   *    ชิ้นจัตุรัสที่ "มีในภาพ" = ใช้เลขในตารางตรง ๆ (4×4 = 60 · 5×5 = 40)
   *    ไม่จัตุรัส หรือ จัตุรัสที่ไม่มีในภาพ (5.5) = นับตามผังจริง (เจ้าของร้านเคาะ 15 ก.ย. 69)
   */
  perSheetTiers?: {
    /**
     * 📐 ขนาดเล็กสุดของแถวนี้ (หน่วยเดียวกับที่ลูกค้ากรอก) — แถวในภาพที่ระบุขนาดตรง ๆ ใส่เท่ากับ upTo
     * (4x4 = {from:4,upTo:4}) · แถวที่เป็นช่วงใส่ตามช่วง (7-8 = {from:7,upTo:8})
     * ⚠️ มี from แล้ว "ขนาดที่ไม่อยู่ในแถวไหนเลย" (5.5 · 2.5 · 10.5) จะ **ไม่ใช้ตาราง** ไปนับตามผังจริงแทน
     * เจ้าของร้านเคาะ 15 ก.ย. 69: เดิมปัดขึ้นขั้นถัดไป (5.5 → ขั้น ≤6 = 24) ทั้งที่ Print-Fit วางได้ 28
     * ไม่ใส่ from = พฤติกรรมเดิม (ทุกขนาดที่ ≤ upTo ใช้แถวนี้) — วาชิยังใช้แบบเดิม
     */
    from?: number;
    upTo?: number;
    per: number;
 ...
 *
 * 📐 กติกาที่เจ้าของร้านเคาะ (15 ก.ย. 69) — ตัดสินจาก **ทรงของชิ้นงาน** ไม่ใช่จำนวนช่องที่กรอก:
 *   • จัตุรัส (กว้าง = สูง · รวมถึงกรอกด้านเดียวแล้วเราแทนเป็น L×L) **และขนาดนั้นมีอยู่ในภาพ**
 *     → ใช้เลขในตารางตรง ๆ (4×4 = 60 · 5×5 = 40 · 7-8 = 15)
 *   • ไม่จัตุรัส → **Print-Fit** (ตารางเทียบด้านยาวสุดด้านเดียว ชิ้นแบน ๆ เลยโดนนับต่ำกว่าจริงมาก)
 *   • จัตุรัสที่ไม่มีในภาพ (5.5 · 2.5 · 10.5) → **Print-Fit** เหมือนกัน (เดิมปัดขึ้นขั้นถัดไปแล้วบอกต่ำกว่าจริง)
 */
/**
 * 📋 เลขในตารางของร้านสำหรับชิ้นจัตุรัสด้านยาวสุด size — null = ขนาดนี้ไม่มีในตาราง (ให้ไปนับตามผังจริง)
 * แถวที่มี from = ใช้เฉพาะขนาดในช่วง [from, upTo] · แถวเก่าที่ไม่มี from = ทุกขนาด ≤ upTo (พฤติกรรมเดิม)
 */
export function perSheetFromTier(cfg: SheetYield, size: number): number | null {
  for (const t of cfg.perSheetTiers ?? []) {
    if (!(t.per > 0)) continue;
    if (t.from != null && size < t.from - 1e-9) continue;
    if (t.upTo != null && size > t.upTo + 1e-9) continue;
    return t.per;
  }
  return null;
}
...
export function sheetFitCount(cfg: SheetYield, w: number, h: number, bound?: SheetBound): number {
   ...
   *
   * ⚠️ ตารางเทียบ "ด้านยาวสุด" ด้านเดียว = คิดเหมือนชิ้นจัตุรัส (ดู SheetYield.longestOnly)
   * รู้ขนาดจริงครบสองด้านแล้วต้องจัดวางตามรูปจริง ไม่งั้นชิ้นแบน ๆ โดนนับต่ำกว่าความจริงมาก
   * (13.97 × 7 ซม. ตาราง 6 ชิ้น/แผ่น A3 · วางจริง 10 — เจ้าของร้านทัก 15 ก.ย. 69 QT-260915-8810)
   */
  const fit = packSingleSize(w, itemH, cfg.sheetW, cfg.sheetH, cfg.gap ?? 0, cfg.printFitOnly);
  /*
   * 📋 ใช้เลขในตารางของร้าน "ตรง ๆ" ไม่กดด้วยผังจริง — เจ้าของร้านย้ำ 15 ก.ย. 69:
   * "ถ้ากรอกแค่ 4 ซม. จะต้องได้ 60 ชิ้น ตามภาพ" (เดิมกดเหลือ 54 ตามผัง Print-Fit)
   * ตารางคือตัวเลขที่ร้านใช้คุยกับลูกค้า ต้องตรงกับภาพที่ส่งให้ลูกค้าดูเป๊ะ ๆ
   * แต่ใช้เฉพาะ "ขนาดที่อยู่ในภาพ" — ขนาดอื่น (5.5 · 2.5 · 10.5) ตกมานับตามผังจริง ให้ตรง Print-Fit
   */
  if (!(cfg.longestOnly && Math.abs(w - itemH) > 1e-9)) {
    const t = perSheetFromTier(cfg, Math.max(w, itemH));
    if (t != null) return t;
  }
  const n = fit;
  /*
...
export function sheetYieldByTable(
  ...
  if (w > 0 && h > 0 && Math.abs(w - h) > 1e-9) return false; // ไม่จัตุรัส = นับตามผังจริง
...
export interface SizeFee {
   ...
   * 📐 ชี้มาที่ **กลุ่มเมนูเลื่อนที่ชื่อตัวเลือกเป็นตัวเลข** ก็ได้ (อ่านตัวเลขจากค่าที่เลือก ตัดตัวอักษรทิ้ง
   * "8cm" → 8) — ชิ้นงานจัตุรัสใส่กลุ่มเดียวกันทั้งคู่ เช่น ค่าสกรีน 2 ด้านของ Carabiner Acrylic
   * ที่คิดตามขนาดที่เลือกจากเมนู (scripts/carabiner-screen-2side-by-size.mjs)
   */
  widthLabel: string;
  heightLabel: string;
...
export interface InputFee {
   ...
   * (ตารางจุดไดคัทของร้าน 9 ก.ย. 69 ฟรี/มากสุด: ≤7 ซม. 5/10 · ≤11 12/20 · ≤15 25/50 · ≤21 50/70 · ≤29.7 (A4) 100/200 · ใหญ่กว่านั้น (A3) 200/500)
   */
  freeBySize?: {
    widthLabel: string;
    heightLabel: string;
 ...
 * tierQty = ยอดที่ใช้เช็ค "ยังอยู่ช่วงปลีกคละอิสระไหม" (ดู designFeeBase) — บรรทัดเดี่ยวเท่ากับ qty
 * แต่ตะกร้าที่รวมล็อตต้องส่งยอดรวมล็อตมา ไม่งั้นบรรทัดเล็กในล็อตที่พ้นช่วงปลีกแล้วได้โควตาฟรีเกินจริง
 */
export function includedDesigns(rate: PriceRate, qty: number, perUnit = 1, tierQty = qty): number {
  if (!rate.minPerDesign || rate.minPerDesign <= 0) return 0;
  // ช่วงปลีกคละอิสระ — ทุกชิ้นเป็นคนละลายได้ ไม่คิดเพิ่ม
  if (isFreeMix(rate, tierQty)) return qty * Math.max(1, perUnit);
...
export function tierQtyFor(product: Product, selections: Record<string, string>, qty: number): number {
  ...
  if (r?.minPerDesign) {
    ...
    // เรทที่คิดค่าคละเกินโควตาเป็นเงินแล้ว (ชิ้นละ underMinPieceFee หรือ ลายละ extraDesignFee)
    // — ห้ามลดเรทซ้ำ ไม่งั้นโดนสองเด้ง: ตกไปราคาปลีก + ยังต้องจ่ายค่าคละอีก
    if (r.underMinPieceFee || r.extraDesignFee) return qty;
...
function mixFeeOfSide(
  ...
  // โควตาฟรีคิดจาก qty ของบรรทัด แต่ "ยังอยู่ช่วงปลีกไหม" ต้องดูยอดรวมล็อต (tierQty)
  const extra = designs - includedDesigns(r, qty, perUnitCapacity(product, selections) ?? 1, tierQty);
...
function ratePoolsFor(
  ...
  for (const r of [...rs].sort((a, b) => (b.minQty ?? 1) - (a.minQty ?? 1))) {
    ...
    const cand = entries
      .map((_, i) => i)
      // เรทที่คิดค่าคละเป็นเงิน (ชิ้นละ underMinPieceFee / ลายละ extraDesignFee) — บรรทัดที่คละไม่ถึง
      // โควตาต่อลายก็เข้าเรทได้ จ่ายค่าคละแทน ไม่ต้องถูกกันออกไปจ่ายราคาปลีก
      .filter(
        (i) =>
          !taken[i] &&
          (per <= 0 ||
...
function usesFreeMixRetail(r: PriceRate | undefined, qty: number, designs: number, perUnit = 1): boolean {
  ...
  // เรทที่คิดค่าคละเกินโควตาเป็นเงิน (ชิ้นละ underMinPieceFee หรือ ลายละ extraDesignFee) — รวมล็อตได้เสมอ
  // (เข้าเรทส่งแล้วส่วนที่เกินโควตาจ่ายค่าคละเอง ไม่ต้องกันออกไปจ่ายราคาปลีก)
  if (r.underMinPieceFee || r.extraDesignFee) return false;
  const per = r.minPerDesign ?? 0;
 ...
 *
 * 📦 ออเดอร์/ใบเสนอราคาที่สั่งไปแล้วก็ต้องเติมเหมือนกัน (ดู itemSel) — ร้านเพิ่มกลุ่ม "ขนาดไดคัท"
 * คั่นหน้าช่องกรอกวันที่ 15 ก.ย. 69 ใบเก่าเลยคำนวณสดไม่ได้เลย (ชิ้น/แผ่นหล่นเป็น 1)
 */
export function backfillShowWhen(p: Product, selections: Record<string, string>): Record<string, string> {
  let out = selections;
  for (const opt of p.options ?? []) {
