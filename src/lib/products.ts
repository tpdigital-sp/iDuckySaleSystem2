...
export interface SheetYield {
   ...
   *    กรอกครบสองด้านและไม่เท่ากัน = ข้ามตาราง ไปจัดวางตามรูปจริง (ดู sheetFitCount)
   */
...
export function sheetFitCount(cfg: SheetYield, w: number, h: number, bound?: SheetBound): number {
  ...
  if (!(cfg.longestOnly && Math.abs(w - itemH) > 1e-9))
    for (const t of cfg.perSheetTiers ?? []) {
      if ((t.upTo == null || Math.max(w, itemH) <= t.upTo) && t.per > 0)
        /*
         * 🧢 ตารางของร้านไล่เป็นขั้น ๆ (≤4 ซม. → 60) ขอบขั้นจึงบอกเกินที่วางได้จริงได้
         * (4 × 4 ซม. ตาราง 60 · Print-Fit วางได้ 54) → ลูกค้าสั่งแผ่นไม่พอ ร้านต้องแถมแผ่น
         * เพดานคือผังจริงเสมอ · ตารางที่ "ต่ำกว่า" ผังจริงยังใช้ได้ (เผื่อ mark/ระยะจับชิ้นของร้าน)
         * วางไม่ลงเลย (fit 0 = งานเต็มแผ่น) ใช้เลขตารางตามเดิม ให้กติกางานเต็มแผ่นข้างล่างจัดการ
         */
        return fit > 0 ? Math.min(t.per, fit) : t.per;
    }
  const n = fit;
  /*
