...
export interface SheetYield {
   ...
   *    ชิ้นจัตุรัส (กว้าง = สูง) = ใช้ตารางตรง ๆ ตามภาพที่ร้านส่งลูกค้า (4×4 = 60 · 5×5 = 40)
   *    ไม่จัตุรัส = ข้ามตาราง ไปนับตามผังจริง (เจ้าของร้านเคาะ 15 ก.ย. 69 — ดู sheetFitCount)
   */
 ...
 *   • จัตุรัส (กว้าง = สูง · รวมถึงกรอกด้านเดียวแล้วเราแทนเป็น L×L) → **ใช้ตารางในภาพตรง ๆ** 4×4 = 60 · 5×5 = 40
 *   • ไม่จัตุรัส → **ใช้จำนวนตาม Print-Fit** (ตารางเทียบด้านยาวสุดด้านเดียว ชิ้นแบน ๆ เลยโดนนับต่ำกว่าจริงมาก)
 */
export function sheetFitCount(cfg: SheetYield, w: number, h: number, bound?: SheetBound): number {
  ...
  if (!(cfg.longestOnly && Math.abs(w - itemH) > 1e-9))
    for (const t of cfg.perSheetTiers ?? []) {
       ...
       * "ถ้ากรอกแค่ 4 ซม. ด้านกว้าง จะต้องได้ 60 ชิ้น ตามภาพ" (เดิมกดเหลือ 54 ตามผัง Print-Fit)
       * ตารางนี้คือตัวเลขที่ร้านใช้คุยกับลูกค้าอยู่แล้ว ต้องตรงกับภาพที่ส่งให้ลูกค้าดูเป๊ะ ๆ
       * (ชิ้นไม่จัตุรัสค่อยไปนับตามผังจริง — ดูหัวข้อบนสุดของฟังก์ชัน)
       */
