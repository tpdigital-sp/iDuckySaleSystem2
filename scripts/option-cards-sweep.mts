 ...
 *   • display ปุ่มแยก (pills/ไม่ตั้ง) · ตัวเลือก 2-5 ตัว
 *   • ทางที่ 1: มีภาพประกอบอย่างน้อย 2 ตัว + ชื่อกลุ่มเชิงสไตล์
 *     (แบบ/ชนิด/เนื้อ/ผิว/เคลือบ/วัสดุ/ทรง/ฐาน/สกรีน/ประเภท/รูปทรง/ชิ้นงาน/เจาะรู/สี)
 *   • ทางที่ 2 (รอบ 2 — ผู้ใช้สั่งไล่เพิ่ม 25 ส.ค. 69): ไม่มีภาพ แต่ชื่อกลุ่มเข้าข่าย
 *     สกรีน/เจาะรู/ชิ้นงาน/เทคนิค และ "ทุกตัวเลือก" มีคำอธิบายในพจนานุกรม → การ์ดข้อความล้วน
...
const STYLE_LABEL = /แบบ|ชนิด|เนื้อ|ผิว|เคลือบ|วัสดุ|ทรง|ฐาน|สกรีน|ประเภท|รูปทรง|ชิ้นงาน|เจาะรู|สี/;
/** กลุ่มไม่มีภาพ แปลงได้เฉพาะแนวนี้ และต่อเมื่อทุกตัวเลือกมีคำอธิบายในพจนานุกรม */
const TEXT_ONLY_LABEL = /สกรีน|เจาะรู|ชิ้นงาน|เทคนิค/;
...
const DESC: Record<string, string> = {
  ...
  // สกรีนกี่ด้าน (งานทั่วไป — พิมพ์หน้าเดียว/สองหน้า)
  "สกรีน 1 ด้าน": "พิมพ์ลายด้านเดียว",
  "สกรีน 2 ด้าน": "พิมพ์ลายทั้งสองด้าน หน้า-หลังคนละลายได้",
  "สกรีนด้านเดียว": "พิมพ์ลายด้านเดียว",
  "ราคาสกรีน 1 ด้าน": "พิมพ์ลายด้านหน้าด้านเดียว",
  "ราคาสกรีน 2 ด้าน": "พิมพ์ลายทั้งสองด้าน หน้า-หลังคนละลายได้",
  "1 ด้าน": "พิมพ์ลายด้านเดียว",
  "2 ด้าน": "พิมพ์ลายทั้งสองด้าน หน้า-หลังคนละลายได้",
  "ไม่สกรีน": "ไม่พิมพ์ลายด้านนี้",
  // งานสกรีนอะคริลิค (ใต้ = แบบมาตรฐานของร้าน)
  "สกรีน 1 ด้าน (ใต้)": "พิมพ์ใต้แผ่น มองลายผ่านเนื้ออะคริลิคใส ผิวเรียบเงา ลายไม่ถลอก — แบบมาตรฐานของร้าน",
  "สกรีน 1 ด้าน (บน)": "พิมพ์บนผิวหน้า ลายคมชัด สัมผัสเนื้อลายได้",
  "สกรีน 2 ด้าน (ใต้-บน)": "พิมพ์สองหน้า — ฝั่งหนึ่งใต้แผ่น อีกฝั่งบนผิว",
  "สกรีน 2 ด้าน (บน-บน)": "พิมพ์บนผิวทั้งสองหน้า",
  "สกรีน 3 เลเยอร์": "พิมพ์ซ้อน 3 ชั้น เพิ่มมิติความลึกให้ลาย",
  "ไม่สกรีน (อะคริลิคเปล่า)": "อะคริลิคเปล่า ไม่พิมพ์ลาย",
  "1 ด้าน (ใต้)": "พิมพ์ใต้แผ่น มองลายผ่านเนื้ออะคริลิคใส ผิวเรียบเงา ลายไม่ถลอก — แบบมาตรฐานของร้าน",
  "1 ด้าน (บน)": "พิมพ์บนผิวหน้า ลายคมชัด สัมผัสเนื้อลายได้",
  "2 ด้าน (ใต้-บน)": "พิมพ์สองหน้า — ฝั่งหนึ่งใต้แผ่น อีกฝั่งบนผิว",
  "2 ด้าน (บน-บน)": "พิมพ์บนผิวทั้งสองหน้า",
  "ด้านใต้": "พิมพ์ใต้แผ่น มองลายผ่านเนื้อใส ผิวเรียบ ลายไม่ถลอก",
  "ด้านบน": "พิมพ์บนผิวหน้า ลายคมชัด สัมผัสเนื้อลายได้",
  "2 เลเยอร์": "พิมพ์ซ้อนสองชั้น เพิ่มมิติความลึกให้ลาย",
  // เจาะรู
  "เจาะรู": "เจาะรูชิ้นงาน พร้อมร้อยโซ่/เชือก/ตะขอ",
  "ไม่เจาะรู": "ชิ้นงานเรียบ ไม่เจาะรู",
  "เจาะรู (สำหรับแขวน)": "เจาะรูสำหรับร้อยเชือก/แขวนโชว์",
};
...
for (const row of data!) {
  ...
  for (const opt of product.options) {
    ...
    if (choices.length < 2 || choices.length > 5) continue;
    const withImg = choices.filter((c) => c.imageSrc).length;
    // ไม่มีภาพพอ → การ์ดข้อความล้วน เอาเฉพาะกลุ่มสกรีน/เจาะรู ที่พจนานุกรมอธิบายได้ครบทุกใบ
    if (withImg < 2 && !(TEXT_ONLY_LABEL.test(opt.label) && choices.every((c) => c.desc || DESC[c.name]))) continue;
