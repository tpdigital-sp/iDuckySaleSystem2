 ...
 *
 * OPTION เสริม (ตารางกราฟิก "ราคาตัดแบ่งผ้า และเย็บ/โพ้ง" — คิดราคาจากด้านที่ยาวที่สุด):
 *   บริการ 3 แบบ ตัดแบ่ง / เย็บขอบ / โพ้งขอบ — ลูกค้าต้อง "ระบุขนาดชิ้นงาน" ก่อนถึงจะคิดราคาได้
 *   (ผู้ใช้สั่ง 25 ส.ค. 69) → ช่องกรอกขนาด กว้าง×ยาว (บังคับกรอก) + กลุ่มขั้นขนาดตามด้านยาวสุด
 *   แยกกลุ่มต่อบริการเพราะ extra ต่างกัน: ตัดแบ่ง +5..25 · เย็บขอบ +15..75 · โพ้งขอบ +10..70 ต่อชิ้น
 *   (ระบบคิด extra ต่อหลาที่สั่ง — ตัดหลายชิ้นต่อหลา แอดมินคิดเพิ่มตามจำนวนชิ้นจริง มี note บอก)
 *   เจาะรูตาไก่ คู่ละ 10 บาท (multi + ระบุจำนวนคู่)
 */
...
const OPTIONS: ProductOption[] = [
  ...
  },
  {
    label: GROUP_EDGE,
    display: "cards",
    note: "ค่าตัดแบ่ง / เย็บขอบ / โพ้งขอบ **คิดราคาจากด้านที่ยาวที่สุด**ของชิ้นงาน — เลือกบริการแล้วระบุขนาดชิ้นงานด้านล่าง · การแบ่งผ้าใน 1 หลา ต้องเหลือพื้นที่สำหรับตัดตกอย่างน้อย 1-2 ซม.",
    choices: [
      { name: "ตัดเต็มหลา ไม่เก็บขอบ", badge: "ฟรี", imageSrc: art["edge-none"], desc: "รับผ้าเต็มผืน 145×90 ซม. ขอบตัดเรียบ ไม่เย็บเก็บริม" },
      { name: "ตัดแบ่ง (ขอบไม่เย็บ)", imageSrc: art["photo-cut-table"], desc: "ตัดแบ่งผ้าเป็นชิ้นตามขนาดที่ระบุ ขอบตัดเรียบไม่เย็บ · ค่าตัดคิดตามขนาดชิ้นงาน" },
      { name: "เย็บขอบ", imageSrc: art["edge-sew"], desc: "พับเข้าเก็บขอบ ด้านละ 1 ซม. ขอบเรียบหนา เก็บริมเรียบร้อย · คิดตามขนาดชิ้นงาน" },
      { name: "โพ้งขอบ", imageSrc: art["edge-serge"], desc: "เย็บโพ้งริมผ้า 4 ด้าน กันลุ่ย ขอบบางกว่าเย็บขอบ · คิดตามขนาดชิ้นงาน" },
    ],
  },
  {
    // ลูกค้าต้องระบุขนาดชิ้นงานก่อน ถึงจะคิดราคาได้ (เก็บลง selections ให้ฝ่ายผลิตด้วย)
    label: "ขนาดชิ้นงาน กว้าง×ยาว",
    display: "input",
    standardInput: true, // ข้อมูลประกอบของงานปกติ — ไม่ตั้ง = โดนจัดเป็น "งานสั่งทำ" ซ่อนใต้กล่อง 📐
    showWhen: { label: GROUP_EDGE, choices: ["ตัดแบ่ง (ขอบไม่เย็บ)", "เย็บขอบ", "โพ้งขอบ"] },
    input: {
      kind: "text",
      placeholder: "เช่น 45×45 ซม.",
      hint: "ระบุ กว้าง×ยาว ของชิ้นงานเป็นเซนติเมตร (ไม่เกิน 145×90 ซม. ต่อ 1 หลา) — ราคาคิดจากด้านที่ยาวที่สุด แล้วเลือกช่วงขนาดให้ตรงด้านล่าง",
      required: true,
      maxLength: 60,
    },
    choices: [],
  ...
  ...(
    [
      ["ตัดแบ่ง (ขอบไม่เย็บ)", "ช่วงขนาดตัดแบ่ง (ด้านที่ยาวที่สุด)", [5, 10, 15, 20, 25]],
      ["เย็บขอบ", "ช่วงขนาดเย็บขอบ (ด้านที่ยาวที่สุด)", [15, 30, 45, 60, 75]],
      ["โพ้งขอบ", "ช่วงขนาดโพ้งขอบ (ด้านที่ยาวที่สุด)", [10, 25, 40, 55, 70]],
    ] as [string, string, number[]][]
  ).map(([service, label, fees]): ProductOption => ({
    label,
    display: "pills",
    showWhen: { label: GROUP_EDGE, choices: [service] },
    note: "ราคาตามตาราง \"ราคาตัดแบ่งผ้า และเย็บ/โพ้ง\" (ดูภาพในแกลเลอรี) คิด**ต่อชิ้น** — ระบบคิดต่อ 1 หลาที่สั่ง ถ้าตัดแบ่งมากกว่า 1 ชิ้นต่อหลา แจ้งจำนวนชิ้นในหมายเหตุถึงร้าน แอดมินจะคิดเพิ่มให้ก่อนยืนยันผลิต",
    choices: [30, 60, 90, 120, 150].map((cm, i) => ({
      name: cm === 150 ? "ไม่เกิน 150 ซม. (เต็มหลา)" : `ไม่เกิน ${cm} ซม.`,
      extra: fees[i],
    })),
  })),
  {
    label: GROUP_EYELET,
    display: "multi",
    note: "เจาะรู + ใส่ตาไก่ สำหรับร้อยเชือก/ตะขอแขวนผนัง — ระบุตำแหน่งที่ต้องการเจาะในช่องหมายเหตุถึงร้าน",
    choices: [{ name: "เจาะรู + ใส่ตาไก่", extra: 10, qty: true, qtyUnit: "คู่", imageSrc: art["eyelet"] }],
  },
];
...
const product: Product = {
  ...
  terms: [
    ...
    "*ค่าตัดแบ่ง / เย็บขอบ / โพ้งขอบ คิดต่อชิ้นตามด้านที่ยาวที่สุดของชิ้นงาน (ตารางราคาอยู่ในแกลเลอรี) — เลือกบริการแล้วระบุขนาดชิ้นงาน ระบบคิดราคาตามช่วงขนาดที่เลือก · ตัดแบ่งมากกว่า 1 ชิ้นต่อหลา แจ้งจำนวนชิ้นในหมายเหตุถึงร้าน",
    "*เจาะรู + ใส่ตาไก่ คู่ละ 10 บาท",
  ...
  tabs: [
    ...
    },
    {
      ...
      text: [
        ...
        "เจาะรู + ใส่ตาไก่ สำหรับแขวนผนัง คู่ละ 10 บาท",
        "",
        "หมายเหตุ::",
        "• การแบ่งผ้าใน 1 หลา ต้องเหลือพื้นที่สำหรับตัดตกอย่างน้อย 1-2 ซม.",
        "• ผ้าตัดเย็บแต่ละชิ้น ขนาดคลาดเคลื่อน ±0.5-1 นิ้ว",
        "• เลือกบริการในหน้าสินค้าแล้วระบุขนาดชิ้นงาน (กว้าง×ยาว) ระบบคิดราคาตามช่วงขนาด — ตัดแบ่งมากกว่า 1 ชิ้นต่อหลา แจ้งจำนวนชิ้นในหมายเหตุถึงร้าน แอดมินจะคิดเพิ่มให้ก่อนยืนยันผลิต",
      ].join("\n"),
    },
    {
      title: "วิธีสั่งงาน",
  ...
  seo: {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: "ได้ — เลือกบริการตัดแบ่ง/เย็บขอบ/โพ้งขอบในหน้าสินค้า แล้วระบุขนาดชิ้นงาน ระบบคิดราคาต่อชิ้นตามด้านที่ยาวที่สุด เริ่มชิ้นละ +5 บาท (ตัดแบ่ง ≤30 ซม.) จนถึง +75 บาท (เย็บขอบ ≤150 ซม.) การแบ่งผ้าใน 1 หลาต้องเหลือพื้นที่ตัดตกอย่างน้อย 1-2 ซม.",
      },
      {
        q: "แขวนผนังยังไง มีเจาะรูให้ไหม?",
        a: "มีบริการเจาะรู + ใส่ตาไก่ คู่ละ 10 บาท สำหรับร้อยเชือกหรือเกี่ยวตะขอแขวนผนัง ระบุจำนวนคู่และตำแหน่งที่ต้องการในหมายเหตุถึงร้าน",
      },
      {
