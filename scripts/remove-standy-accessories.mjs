...
const REMOVE = [
  "สปริง (ตัวโยก)",
  "แปะกาวหนีบรูปด้านหลัง",
  "แปะกาวใส่รูปด้านหลัง",
  "จุกยางหมุนได้ (ชุด)",
  "เซาะร่อง ติดจุดหมุน",
];

/**
 * บรรทัดในแท็บ Add-on ที่ไล่ราคาอุปกรณ์เสริม — เขียนทับทั้งบรรทัดด้วยข้อความนี้
 * (ไม่จับคู่ข้อความเดิมแบบเป๊ะ ๆ เพราะถอดของหลายรอบ บรรทัดเดิมเปลี่ยนไปทุกครั้ง)
 * ⚠️ ถอดของเพิ่มเมื่อไหร่ ต้องแก้บรรทัดนี้ให้ตรงกับตัวเลือกที่เหลือด้วย — สคริปต์ตรวจให้ตอนท้าย
 */
const TAB_TITLE = "Add-on / อุปกรณ์เสริม";
const TAB_LINE =
  "• อุปกรณ์เสริม: จี้ห้อยหัวใจ คู่ละ 15.- · NFC 20.- · แม่เหล็ก 5.- · เซาะร่องติดแม่เหล็ก 3mm จุดละ 15.-";
...
const accLine = /^• อุปกรณ์เสริม:.*$/m;
if (!tab || !accLine.test(tab.text)) throw new Error(`ไม่เจอบรรทัด "• อุปกรณ์เสริม:" ในแท็บ "${TAB_TITLE}" — ไม่บันทึก`);
if (tab.text.match(accLine)[0] === TAB_LINE) {
  console.log(`   แท็บ "${TAB_TITLE}": ตรงอยู่แล้ว`);
} else {
  tab.text = tab.text.replace(accLine, TAB_LINE);
  console.log(`   แท็บ "${TAB_TITLE}": เขียนรายการราคาใหม่แล้ว`);
}
