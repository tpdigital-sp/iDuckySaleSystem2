/**
 * 🎨 ปุ่มลอยมุมขวาทั้งกอง รอบ v3 — ใช้ "ภาพชุดใหม่ของเจ้าของร้าน" ทั้ง 4 ใบ
 *    (เจ้าของร้านส่งภาพรวมกองมา 22 ก.ย. 69: "นำภาพที่ 1 ไปใส่แทนที่ภาพที่ 2")
 *
 * ต้นฉบับคือภาพใบเดียวที่มีปุ่มเรียงกัน 4 ใบ พื้นหลังโปร่ง:
 *   scripts/assets/fab-badges/fab-stack-owner-v3.webp
 * สคริปต์นี้แค่ "ตัดแยก" ออกเป็นไฟล์ละใบ ไม่ได้แต่งสี/ย่อขยาย — ของที่ได้จึงตรงภาพต้นฉบับเป๊ะ
 *
 *   node scripts/fab-badges-v3.mjs         → เขียนไฟล์ + พิมพ์ตัวเลขสำหรับ landing.css
 *   node scripts/fab-badges-v3.mjs --dry   → วัดอย่างเดียว ไม่เขียนไฟล์
 *
 * ⚠️ เซฟเป็นชื่อใหม่ (-v3) ไม่ทับไฟล์เดิม — ทับพาธเดิมแล้ว CDN/เบราว์เซอร์ยังจ่ายรูปเก่า
 *    ชุด -v2 (และของก่อนหน้า) ยังอยู่ครบ ย้อนกลับได้ด้วยการชี้ชื่อไฟล์กลับ
 *
 * ── ทำไมต้องพิมพ์ตัวเลขออกมา ───────────────────────────────────────────────
 * กองปุ่มมุมขวาต้องกว้าง "เท่ากันทุกใบ" (แคปซูล 208px · ห่างขอบขวา 20px · ห่างกัน 12px)
 * แต่ในไฟล์ภาพ แต่ละใบมีเป็ด/ขีดกระเด็นโผล่พ้นขอบแคปซูลคนละระยะ
 * ถ้าเอาขอบ "ไฟล์" ไปชิดขอบจอตรง ๆ แคปซูลจะเหลื่อมกันทันที
 * สคริปต์จึงวัดว่า "ตัวแคปซูล" อยู่ตรงไหนในไฟล์ แล้วคำนวณ width/right/bottom ให้เสร็จ
...
const SRC = path.join(HERE, "assets", "fab-badges", "fab-stack-owner-v3.webp");
...
const NAMES = ["cart-shop-badge-v3", "bot-chat-badge-v3", "line-chat-badge-v3", "admin-home-badge-v3"];
