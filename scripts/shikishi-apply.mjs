...
d.highlights = [
  ...
  `เคลือบเงา/ด้าน +฿${COAT_FEE} · เคลือบพิเศษ +฿${SPECIAL_FEE} · เคลือบฟอยล์ +฿${FOIL_1.price}`,
];
...
d.terms = [
  ...
  `งานเคลือบและงานฟอยล์ทำเป็นแผ่น A3 — ${SIZES.filter((s) => s.perA3).map((s) => `${s.key} ได้ ${s.perA3} ใบ`).join(" · ")} ต่อ 1 แผ่น A3`,
...
d.tabs = [
  ...
  },
  {
    ...
    text: [
      `• สั่งตั้งแต่ ${MIX_FROM} ชิ้นขึ้นไป คละลาย / คละขนาดได้ — ขั้นต่ำลายละ ${MIX_MIN} ชิ้น`,
      `• งานเคลือบและงานฟอยล์ทำเป็นแผ่น A3 (${SIZES.filter((s) => s.perA3).map((s) => `${s.key} ${s.perA3} ใบ`).join(" · ")} ต่อแผ่น)`,
...
d.priceMin = MIN;
d.priceMax = MAX + SPECIAL_FEE + FOIL_2.price + HOLO_FEE;
