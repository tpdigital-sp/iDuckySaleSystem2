...
const FIMO_YES = "ใส่ Fimo (ดาว/ไข่มุก/เส้น)";
...
const TABS: Product["tabs"] = [
  {
    ...
    text: [
      ...
      "• Add On อะคริลิคตัวน้อย (ขนาด 1.5-2 ซม. หนา 1.5 mm) บวกเพิ่มชิ้นละ 15 บาท",
      "• Fimo ตัวน้อยเขย่า มี 3 แบบ — ดาว | ไข่มุก | เส้น ใส่ให้ฟรี · กำหนดปริมาณไม่ได้ (ระบุแบบที่ชอบได้ในหมายเหตุถึงร้าน)",
...
const product: Product = {
  ...
  options: [
    ...
    },
    {
      ...
      choices: [
        { name: "ไม่เพิ่ม" },
        { name: MINI_YES, extra: 15, imageSrc: IMG("addon-mini") },
      ],
    },
    {
      ...
      choices: [
        { name: FIMO_YES, popular: true, imageSrc: IMG("fimo-mix") },
        { name: FIMO_NO },
      ],
    },
  ],
  terms: [
    "1-10 ชิ้น สามารถคละลายได้ · ตั้งแต่ 11 ชิ้นขึ้นไป คละลาย คละขนาด ขั้นต่ำ 5 ชิ้น",
    "ฐานสีดำและสีขาว ไม่บวกเงินเพิ่ม · เฉพาะฐานใส บวกเพิ่ม 5 บาท",
    "อะคริลิคตัวน้อย ขนาด 1.5-2 ซม. หนา 1.5 mm บวกเพิ่มชิ้นละ 15 บาท",
    "Fimo ตัวน้อยเขย่า (ดาว/ไข่มุก/เส้น) ใส่ให้ฟรี · ไม่สามารถกำหนดปริมาณได้ · ระบุแบบที่ชอบได้ในหมายเหตุถึงร้าน",
    "สั่งตั้งแต่ 24 ชิ้นขึ้นไป ฟรีแพ็คเกจ (คละลาย/ขั้นต่ำ 24 30 36 42 ... บวกเพิ่มทีละ 6 ชิ้น ไม่รับเศษที่หาร 6 ไม่ลงตัว)",
    "ตัดตกจากขนาดงานจริงด้านละ 3mm · ขนาดชิ้นงานนับจากด้านที่ยาวที่สุด (ไม่วัดแนวทแยง)",
...
async function main() {
  ...
  const { data: row } = await sb.from("products").select("id,name").eq("id", ID).maybeSingle();
