 ...
 *
 * ⚠️ เปลี่ยนกติกาสำคัญจากของเดิม: **ค่าฐานคิดทุกช่วงจำนวน** ตามตารางฐานตรง ๆ
 *    ของเดิมยกกติกาสแตนดี้ปกติมา คือช่วงปลีก 1-10 ชิ้นไม่คิดค่าฐาน (extraFromQty 11 + extraBelow)
 *    ซึ่งทำให้ฐาน 6cm คิด 0 แต่ฐาน 7cm คิด 5 ทั้งที่ตารางร้านตั้งเท่ากัน (15) — ผู้ใช้เลือกให้คิดตามตาราง
 *    ผลคือช่วง 1-10 ชิ้นแพงขึ้นเท่าค่าฐาน (เช่น ตัว 6cm + บน 2cm + ฐาน 3cm: 170 → 180)
...
/* ══ 4. ค่าฐาน — คิดทุกช่วงจำนวนตามตารางราคาฐาน ═══════════════════════ */
const baseSizeGroup = groupOf(BASE_SIZE);
delete baseSizeGroup.extraFromQty; // เลิกกติกา "ช่วงปลีกไม่คิดค่าฐาน"
...
/* ══ 7. ข้อความที่บอกว่า "ราคารวมฐานแล้ว" — ต้องแก้ให้ตรงกับกติกาใหม่ ══ */
...
for (const [ownerFn, key, from, to] of swaps) {
  const owner = ownerFn();
  ...
  if (!owner[key].includes(from)) throw new Error(`ข้อความเดิมเปลี่ยนไปแล้ว หาไม่เจอ:\n"${from}"\nในค่า:\n"${owner[key].slice(0, 300)}"`);
...
console.log(`   ค่าฐาน  : คิดทุกช่วงจำนวน — ${baseSizeGroup.choices.map((c) => `${c.name} +${c.extra}`).join(" · ")}`);
...
for (const [qty, rate, tier, quote] of [
  ...
  const tag = unit === quote ? "= ใบเสนอราคา ✅" : `≠ ใบเสนอราคา ${money(quote)} (ต่าง ${unit - quote > 0 ? "+" : ""}${unit - quote} — ค่าฐานที่เดิมฟรีช่วงปลีก)`;
