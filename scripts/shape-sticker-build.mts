 ...
 *   แบบไดคัท = "ไดคัท 50% (ตัดครึ่ง · ลอกทีละดวง)" (ทุกชนิดสติ๊กเกอร์)
 * เหลือ 2 แกนคือ ชนิดสติ๊กเกอร์ × การเคลือบ × 9 ขั้นจำนวน (อ่านสดจาก Supabase ทุกครั้งที่รัน)
 *
 * ทำไมสไลซ์นี้: สติ๊กเกอร์รูปทรงคืองานตัดครึ่งเนื้อ ลอกทีละดวง = ไดคัท 50%
 * และราคาขั้นแรกของ PP ขาวมันตรงกับหน้าร้านจริงพอดี (90 บาท/แผ่น A3) — สคริปต์เช็คให้ทุกครั้ง (ดู CROSS-CHECK ข้างล่าง)
 *
 * ⚠️ 1 ก.ย. 69 (ผู้ใช้สั่งเพิ่ม): เดิมล็อกไว้เนื้อเดียว (PP ขาวมัน) ตอนนี้เปิดให้เลือกครบ 5 เนื้อเหมือนสติ๊กเกอร์ Digital
 *    → ตารางราคากลายเป็น 2 แกน · เนื้อ "PP ใส (รองขาว) +20฿" แพงกว่าเนื้ออื่นทุกขั้น (บวกไว้ในตารางแล้ว ไม่ใช่ค่าบวกแยก)
 ...
 *   • "ชนิดสติ๊กเกอร์" — ยกการ์ด (ชื่อ · คำอธิบาย · ภาพ) มาจาก sticker-pp ทั้งชุด อ่านสดจาก DB
...
const BASE_MATERIAL = "PP ขาวมัน"; // เนื้อมาตรฐาน — ใช้อ้างอิงตัวเลขในคำอธิบาย/SEO
...
/** การ์ดชนิดสติ๊กเกอร์ของ sticker-pp (ชื่อ · คำอธิบาย · ภาพ) — อ่านสด ไม่ hardcode */
const srcMatOpt = (src.options ?? []).find((o: ProductOption) => o.label === MAT_LABEL);
if (!srcMatOpt?.choices?.length) throw new Error(`ไม่เจอกลุ่ม "${MAT_LABEL}" ใน ${FROM} — โครงต้นทางเปลี่ยน`);
const MATERIALS: string[] = srcMatOpt.choices.map((c: { name: string }) => c.name);

/** ตารางราคา 2 แกน: ชนิดสติ๊กเกอร์ × การเคลือบ (คีย์ "เนื้อ│เคลือบ") */
...
/** ราคาของเนื้อมาตรฐาน (PP ขาวมัน) ขั้นที่ i — ใช้เขียนคำอธิบาย/SEO */
const P = (coat: (typeof COATS)[number], i: number) => CELLS[`${BASE_MATERIAL}│${coat}`][i];
const LAST = TIERS.length - 1;
/** ส่วนต่างของเนื้อที่แพงกว่าเนื้อมาตรฐานในขั้นแรก (0 = ราคาเท่ากัน) */
const matDiff = (mat: string) => CELLS[`${mat}│ไม่เคลือบ`][0] - P("ไม่เคลือบ", 0);
console.log(
  `💰 ราคายกมาจาก "${srcRow.name}" — ${DIECUT} · ${MATERIALS.length} เนื้อ × ${COATS.length} เคลือบ × ${TIERS.length} ขั้น (หน่วย ${srcPricing.unit})`
);
...
const same = siteBase === P("ไม่เคลือบ", 0) && siteGloss === P("เคลือบเงา", 0);
console.log(
...
/**
 * ชนิดสติ๊กเกอร์ — แกนตารางราคาแกนแรก (ผู้ใช้สั่งเพิ่ม 1 ก.ย. 69)
 * ยกการ์ดทั้งชุดมาจากสติ๊กเกอร์ Digital: ชื่อ/คำอธิบาย/ภาพต้องตรงกัน 2 สินค้า
 * ⚠️ ชื่อเนื้อคือคีย์ตารางราคา — ห้ามแก้ชื่อที่นี่ ต้องไปแก้ที่ sticker-pp ให้ตรงกันทั้งคู่
 */
const matGroup: ProductOption = {
  label: MAT_LABEL,
  note:
    `เนื้อสติ๊กเกอร์ PP กันน้ำทุกแบบ — ราคาต่างกันตามเนื้อที่เลือก (ชุดเดียวกับ**สติ๊กเกอร์ Digital**) · ` +
    `ช่วง ${TIERS[0].label} ไม่เคลือบ: ` +
    MATERIALS.map((m) => `${m.replace(/ \+\d+฿$/, "")} ${CELLS[`${m}│ไม่เคลือบ`][0]}`).join(" · ") +
    " บาท/แผ่น A3",
  display: "cards",
  choices: (srcMatOpt.choices as { name: string; desc?: string; imageSrc?: string }[]).map((c) => ({
    name: c.name,
    desc: c.desc,
    imageSrc: c.imageSrc,
    // เนื้อที่แพงกว่ามาตรฐาน ติดป้ายบอกส่วนต่างขั้นแรกไว้บนการ์ด (ตัวเลขบวกอยู่ในตารางแล้ว)
    ...(matDiff(c.name) > 0 ? { badge: `+${matDiff(c.name)} บาท/แผ่น A3` } : {}),
    ...(c.name === BASE_MATERIAL ? { popular: true } : {}),
  })),
};
...
const coatGroup: ProductOption = {
  ...
  note:
    ...
    `ช่วง ${TIERS[0].label} บนเนื้อ ${BASE_MATERIAL}: ไม่เคลือบ ${P("ไม่เคลือบ", 0)} · เงา/ด้าน ${P("เคลือบเงา", 0)} · พิเศษ ${P("เคลือบพิเศษ", 0)} บาท ` +
    `(เนื้ออื่นบวกลบตามตารางราคา)`,
  display: "cards",
  choices: [
...
const TABS: Product["tabs"] = [
  {
    ...
    text: [
      "::เนื้อสติ๊กเกอร์::",
      `• เลือกเนื้อได้ ${MATERIALS.length} แบบ — กระดาษสติ๊กเกอร์ PP กันน้ำ ไม่ฉีกขาดง่าย ลอกใช้ทีละดวงทุกแบบ`,
      ...(srcMatOpt.choices as { name: string; desc?: string }[]).map(
        (c) =>
          `   – ${c.name.replace(/ \+\d+฿$/, "")}: ${c.desc ?? ""}` +
          (matDiff(c.name) > 0 ? ` (แผ่นละ +${matDiff(c.name)} บาท)` : "")
      ),
      ...
      "::ราคา::",
      `• คิดเป็นแผ่น A3 — ราคาชุดเดียวกับสติ๊กเกอร์ Digital (ไดคัท 50%) ลดเป็นขั้นตามจำนวน ${TIERS.length} ขั้น · ราคาขึ้นกับเนื้อ × การเคลือบ`,
      `• ${TIERS[0].label} บนเนื้อ ${BASE_MATERIAL}: ไม่เคลือบ ${P("ไม่เคลือบ", 0)} บาท/แผ่น A3 · เคลือบเงา/ด้าน ${P("เคลือบเงา", 0)} · เคลือบพิเศษ ${P("เคลือบพิเศษ", 0)}`,
      `• สั่งเยอะสุด (${TIERS[TIERS.length - 1].label}) เหลือแผ่นละ ${P("ไม่เคลือบ", LAST)} บาท — ดูราคาทุกขั้นได้ที่ตารางราคาด้านบน`,
...
const product: Product = {
  ...
  price: P("ไม่เคลือบ", LAST), // ราคาต่ำสุดในตาราง (ขั้นสั่งเยอะสุด)
  emoji: "🏷️",
  ...
  badge: "ใหม่",
  description:
    `สติ๊กเกอร์รูปทรง (SHAPE STICKER) พิมพ์ลายตามสั่งบนกระดาษสติ๊กเกอร์ PP กันน้ำ เลือกเนื้อได้ ${MATERIALS.length} แบบ (ขาวมัน ขาวด้าน ขาวมุก ใสรองขาว ใสไม่รองขาว) ระบบ Digital Printing ` +
  ...
  highlights: [
    ...
    `เลือกเนื้อสติ๊กเกอร์ได้ ${MATERIALS.length} แบบ — ${MATERIALS.map((m) => m.replace(/ \+\d+฿$/, "")).join(" · ")}`,
    ...
    "กระดาษสติ๊กเกอร์ PP กันน้ำทุกเนื้อ · พิมพ์ Digital สีคมชัด",
    `ราคาชุดเดียวกับสติ๊กเกอร์ Digital (ไดคัท 50%) — ${P("ไม่เคลือบ", 0)} บาท/แผ่น A3 สั่งเยอะเหลือ ${P("ไม่เคลือบ", LAST)} บาท`,
  ],
  images: [
  ...
  terms: [
    ...
    "• คละลายในแผ่นเดียวกันได้ ค่าคละลายละ 20 บาท ลายแรกไม่คิด (ระบบคิดให้อัตโนมัติ)",
    `• เลือกเนื้อสติ๊กเกอร์ได้ ${MATERIALS.length} แบบ — ราคาต่อแผ่นเปลี่ยนตามเนื้อที่เลือก (ดูตารางราคาด้านบน)`,
  ...
  tabs: TABS,
  seo: {
    title: `รับผลิตสติ๊กเกอร์รูปทรง ${sizes.length} แบบ เลือกเนื้อ PP ${MATERIALS.length} ชนิด | iDucky Prints`,
    description:
      "รับผลิตสติ๊กเกอร์รูปทรง (Shape Sticker) ทรงกลม สี่เหลี่ยม หัวใจ ดาว ดอกไม้ ก้อนเมฆ กรอบรูป " +
      `กระดาษสติ๊กเกอร์ PP กันน้ำ ${MATERIALS.length} เนื้อ (ขาวมัน ขาวด้าน ขาวมุก ใส) เคลือบเงา/ด้าน/กลิตเตอร์/โฮโลแกรม คิดเป็นแผ่น A3 (1 แผ่น A3 ได้ ${SHEETS_PER_A3} แผ่น) เริ่ม ${P("ไม่เคลือบ", LAST)} บาท ไม่มีขั้นต่ำ ส่งทั่วไทย`,
    faqs: [
      {
        ...
        a:
          ...
          `ราคาชุดเดียวกับสติ๊กเกอร์ Digital (ไดคัท 50%) เริ่ม ${P("ไม่เคลือบ", 0)} บาท/แผ่น A3 (${BASE_MATERIAL} ไม่เคลือบ) ` +
          `สั่งเยอะลดเป็นขั้นถึงแผ่นละ ${P("ไม่เคลือบ", LAST)} บาท`,
      },
      {
      ...
      {
        q: `เนื้อสติ๊กเกอร์มีให้เลือกกี่แบบ ต่างกันยังไง?`,
        a:
          `เลือกได้ ${MATERIALS.length} แบบ (ชุดเดียวกับสติ๊กเกอร์ Digital): ` +
          (srcMatOpt.choices as { name: string; desc?: string }[])
            .map(
              (c) =>
                `${c.name.replace(/ \+\d+฿$/, "")} — ${c.desc ?? ""}` +
                (matDiff(c.name) > 0 ? ` (แผ่นละ +${matDiff(c.name)} บาท)` : "")
      ...
      },
      {
        ...
        a:
          `เลือกได้ 4 แบบ (ราคาบนเนื้อ ${BASE_MATERIAL}): ไม่เคลือบ ${P("ไม่เคลือบ", 0)} · เคลือบเงา ${P("เคลือบเงา", 0)} · เคลือบด้าน ${P("เคลือบด้าน", 0)} · ` +
          `เคลือบพิเศษ (กลิตเตอร์ ทราย โฮโลแกรม 10 ลาย) ${P("เคลือบพิเศษ", 0)} บาท/แผ่น A3 ในช่วง ${TIERS[0].label} — สั่งเยอะราคาลดทุกแบบ`,
      },
      { q: "ใช้เวลาผลิตกี่วัน มีขั้นต่ำไหม?", a: "ผลิต 2-3 วันทำการ ไม่รวมจัดส่ง · ไม่มีขั้นต่ำ สั่ง 1 แผ่น A3 ก็ได้" },
    ],
  },
  hidden: true, // ร่างตอนสร้างครั้งแรก — ถ้าแถวจริงเผยแพร่ไปแล้ว ตอนเขียนจะถอดออกให้ (ดูข้างล่าง)
};
...
for (const [k, v] of Object.entries(CELLS)) console.log(`   ${k.padEnd(34)} ${v.join(" / ")}`);
...
const { data: row, error: rowErr } = await sb.from("products").select("id,name,sort,data").eq("id", ID).maybeSingle();
if (rowErr) throw rowErr;
if (row && row.name !== NAME) throw new Error(`id ${ID} เป็นสินค้าอื่นแล้ว: "${row.name}" — หยุดไว้ก่อน`);
// ⚠️ เผยแพร่ไปแล้วห้ามกดกลับเป็นร่าง (รันสคริปต์ทับรอบสองเคยทำสินค้าหลุดจากหน้าร้านมาแล้ว 1 ก.ย. 69)
if (row && !(row.data as { hidden?: boolean } | null)?.hidden) {
  delete (saved as { hidden?: boolean }).hidden;
  console.log("ℹ️  แถวนี้เผยแพร่อยู่แล้ว — คงสถานะเผยแพร่ไว้ (ไม่ตั้ง hidden)");
}
