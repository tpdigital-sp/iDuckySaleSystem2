 ...
 *
 * ราคาจากเว็บ (5 แท็บของบล็อกนั้น) → 4 เรทราคา + แกน "จำนวนจุดที่สกรีน"
 ...
 *   ตาราง "สกรีนมากกว่า 1 จุด" เข้าเป็นแกนที่สอง — เว็บแยกไว้ 2 ชุด
 ...
 *   point-1..3     ผังตำแหน่งสกรีน 1/2/3 จุด (วาดเอง — หน้า/หลัง/แขน)
...
const UNIT = "ตัว";
const SIZE_LABEL = "ขนาดสกรีน";
const POINT_LABEL = "จำนวนจุดที่สกรีน";
...
/**
 * ตารางราคา 2 แกน (ขนาดสกรีน × จำนวนจุด)
 * base = ราคาเสื้อ 1 จุด ต่อขนาด · fee = ตารางจุดที่เพิ่มของระบบพิมพ์นั้น
 * (สั่ง 11-29 ตัว สกรีนตัวละ 1 จุดเพิ่ม = 11-29 จุด จึงใช้ราคาช่วงเดียวกัน)
 */
function matrixWithPoints(base: Record<string, number[]>, fee: Record<string, number[]>) {
  const cells: Record<string, number[]> = {};
  for (const size of SIZES) {
    for (let p = 0; p < POINTS.length; p++) {
      cells[`${size}│${POINTS[p]}`] = base[size].map((price, ti) => price + p * fee[size][FEE_TIER[ti]]);
    }
  }
  return { unit: UNIT, driverLabels: [SIZE_LABEL, POINT_LABEL], tiers: TIERS, cells };
}

// ── เรท 1: DTF/DFT
const DTF = matrixWithPoints({ [S5]: [280, 220, 180, 175], [SA5]: [295, 220, 180, 175] }, POINT_FEE_PRINT);

// ── เรท 2: FLEX — เว็บลงตัวเลขเท่ากับ DTF ทุกช่อง
const FLEX = matrixWithPoints({ [S5]: [280, 220, 180, 175], [SA5]: [295, 220, 180, 175] }, POINT_FEE_PRINT);

// ── เรท 3: ซับลิเมชั่น — ถูกกว่าอีกนิด และมีตารางจุดเพิ่มของตัวเอง
const SUB = matrixWithPoints({ [S5]: [280, 210, 170, 160], [SA5]: [270, 210, 170, 160] }, POINT_FEE_SUB);
...
const TABS: Product["tabs"] = [
  {
    ...
    text:
      ...
      "สกรีนมากกว่า 1 จุด (จุดที่ 2 เป็นต้นไป คิดเพิ่มต่อจุด)::\n" +
  ...
  },
  {
    ...
    text:
      ...
      '• เลือกระบบพิมพ์ ขนาดสกรีน จำนวนจุด และสีเสื้อ แล้วแนบภาพลาย (ไฟล์ตัวอย่าง JPG/PNG) หรือใส่ลิงก์ไฟล์งานในช่อง "แนบลายของคุณ"\n' +
      '• ระบุรายละเอียดเพิ่มเติมในช่อง "หมายเหตุถึงร้าน" เช่น ตำแหน่งสกรีน (อก/หลัง/แขน) · จำนวนแต่ละสี · วันที่ต้องการใช้งาน\n' +
...
const product: Product = {
  ...
  description:
    ...
    "เลือกขนาดสกรีนได้ตั้งแต่ไม่เกิน 5 นิ้ว ถึง A5 และสกรีนได้มากกว่า 1 จุด (อก + หลัง + แขน) " +
  ...
  highlights: [
    ...
    "สกรีนได้มากกว่า 1 จุด (อก + หลัง + แขน) คิดเพิ่มตามขนาดจุดที่เพิ่ม",
  ...
  options: [
    ...
    },
    {
      ...
      showWhen: { label: RATE_LABEL, choices: [RATE_DTF, RATE_FLEX, RATE_SUB] },
      choices: [
        { name: P1, imageSrc: IMG("point-1") },
        { name: P2, imageSrc: IMG("point-2") },
        { name: P3, imageSrc: IMG("point-3") },
      ],
    },
    {
  ...
  terms: [
    ...
    "สกรีนจุดที่ 2 เป็นต้นไป คิดเพิ่มต่อจุดตามขนาด — DTF/Flex: 5 นิ้ว 30-15 บาท · A5 45-28 บาท | ซับลิเมชั่น: 5 นิ้ว 30-15 บาท · A5 40-18 บาท (ยิ่งจำนวนจุดเยอะยิ่งถูก)",
