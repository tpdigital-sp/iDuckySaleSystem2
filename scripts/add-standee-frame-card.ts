 ...
 *    ค่าฐานคิดเพิ่มเฉพาะตั้งแต่ 11 ชิ้นขึ้นไป เหมือนสินค้า "สแตนดี้อะคริลิค" ตัวหลักที่ตั้ง extraFromQty 11
 *
...
const VIDEO_MP4 = `${env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/product-images/products/${ID}/clip-card-${REV}.mp4`;
const VIDEO_POSTER = IMG("clip-card-poster");
 ...
 * ค่าต่างกันตามช่วงจำนวน แต่ไม่ต้องทำเป็นแกนตาราง — ใช้ extra 5 (เรทส่ง) คู่กับ smallQtyFee 10 (ช่วง 1-10 ชิ้น)
 * ⚠️ smallQtyFee "คิดแทน" ราคาตัวเลือก ไม่ใช่บวกซ้ำ (ดู groupAddOf) → 1-10 ชิ้นได้ 10 บาท ไม่ใช่ 15
 */
      ...
      BASES.flatMap((b) =>
        [SCREEN_BASE_NO, SCREEN_BASE_YES].flatMap((baseScreen) =>
          [SCREEN_1, SCREEN_2].map((screen) => [
            ...
            //    ตั้งแต่ 11 ชิ้นขึ้นไป (เรทราคาส่ง) ค่อยบวกค่าฐานตามตาราง "ราคาฐาน สแตนดี้"
            SHEET[cm].map(
              (price, ti) =>
                price +
...
const options: ProductOption[] = [
  ...
  },
  {
    ...
    // ⚠️ ส่วน "กรอบการ์ด" ทางร้านมีแค่แนวตั้งแบบเดียว จึงไม่มีให้เลือก
    label: LAYOUT_LABEL,
    choices: [
...
const TABS: Product["tabs"] = [
  {
    ...
    text:
      ...
      "• วางได้ทั้งแนวตั้งและแนวนอน ราคาเท่ากัน (ขนาดที่สั่งคือด้านที่ยาวที่สุด) · กรอบการ์ดเป็นแนวตั้งแบบเดียว\n" +
  ...
  },
  {
    ...
    // แท็บนี้ใช้ html เพราะมี "คลิปงานจริง" ฝังอยู่ (ตัว text ด้านล่างเก็บไว้เป็นสำรอง
    // เผื่อทีมงานลบ html ทิ้งตอนแก้ในหลังบ้าน — ProductDetail จะกลับไปใช้ text ให้เอง)
    html:
      `<p><strong>กรอบการ์ดคืออะไร</strong></p>` +
      `<ul>` +
      `<li>แผ่นอะคริลิคใสแปะกาวไว้ที่ด้านหลังของตัวสแตนดี้ เว้นช่องด้านบนไว้สอดการ์ดเข้า-ออก</li>` +
      `<li>รองรับการ์ดขนาดมาตรฐาน 5.4 × 8.5 ซม. (ขนาดเดียวกับบัตรพนักงาน/โฟโต้การ์ดทั่วไป)</li>` +
      `<li><strong>ทางร้านทำกรอบการ์ดเป็นแนวตั้งแบบเดียว</strong> — ใช้ได้ทั้งตัวสแตนดี้แนวตั้งและแนวนอน ` +
      `(การ์ดขนาดอื่นแจ้งแอดมินก่อนสั่งได้)</li>` +
      `</ul>` +
      `<p><strong>ราคา</strong></p>` +
      `<ul>` +
      `<li>ค่ากรอบการ์ด 50 บาท/ชิ้น รวมอยู่ในราคาที่แสดงแล้ว (ไม่ต้องบวกเอง)</li>` +
      `<li>ตัวสแตนดี้ต้องขนาด 15 ซม. ขึ้นไป การ์ดถึงจะใส่ได้พอดี</li>` +
      `</ul>` +
      `<p><strong>คลิปงานจริง</strong></p>` +
      `<video controls playsinline preload="metadata" poster="${VIDEO_POSTER}" ` +
      `style="width:100%;max-width:300px;border-radius:18px;background:#0f172a">` +
      `<source src="${VIDEO_MP4}" type="video/mp4"/>` +
      `เบราว์เซอร์ไม่รองรับการเล่นวิดีโอ` +
      `</video>` +
      `<p style="font-size:13px;color:#64748b">คลิปจากหน้าตารางราคาของร้าน — สแตนดี้พร้อมกรอบใส่การ์ดด้านหลัง</p>`,
    text:
      "กรอบการ์ดคืออะไร::\n" +
      ...
      "• ทางร้านทำกรอบการ์ดเป็นแนวตั้งแบบเดียว — ใช้ได้ทั้งตัวสแตนดี้แนวตั้งและแนวนอน " +
      "(การ์ดขนาดอื่นแจ้งแอดมินก่อนสั่งได้)\n\n" +
      "ราคา::\n" +
      "• ค่ากรอบการ์ด 50 บาท/ชิ้น รวมอยู่ในราคาที่แสดงแล้ว (ไม่ต้องบวกเอง)\n" +
      "• ตัวสแตนดี้ต้องขนาด 15 ซม. ขึ้นไป การ์ดถึงจะใส่ได้พอดี",
    images: [IMG("photo-card"), IMG("card-slot"), IMG("photo-addon")],
  ...
  },
  {
    ...
    text:
      ...
      "• วางงานได้ทั้งแนวตั้งและแนวนอน ราคาเท่ากัน — กรอบใส่การ์ดด้านหลังเป็นแนวตั้งทั้งสองแบบ\n\n" +
      "ฐาน::\n" +
  ...
  },
  {
    ...
    text:
      ...
      "• เลือกขนาดตัวสแตนดี้ · แนววางงาน (ตั้ง/นอน) · ขนาดฐาน · สกรีนฐานหรือไม่ · สกรีน 1 หรือ 2 ด้าน · สีอะคริลิค\n" +
      ...
      "• ระบุรายละเอียด: ขนาดตัวสแตนดี้ · แนววางงาน · ขนาด/สกรีนฐาน · สกรีนกี่ด้าน · จำนวน · วันที่ใช้งาน (ถ้ามี)\n" +
  ...
  faqs: [
    {
      ...
      a:
        ...
        "· สั่ง 1-10 ชิ้น ตัว 15 ซม. อยู่ที่ 280 บาท/ชิ้น (รวมฐานและกรอบการ์ดแล้ว) — ยิ่งสั่งเยอะยิ่งถูกตามตารางราคา",
    },
    {
      q: "ใส่การ์ดขนาดไหนได้บ้าง?",
      a: "กรอบมาตรฐานรองรับการ์ดขนาด 5.4 × 8.5 ซม. (เท่าบัตรพนักงาน/โฟโต้การ์ดทั่วไป) — ทางร้านทำกรอบเป็นแนวตั้งแบบเดียว ใช้ได้ทั้งตัวสแตนดี้แนวตั้งและแนวนอน · การ์ดขนาดอื่นแจ้งแอดมินก่อนสั่งได้",
    },
    {
...
const product: Product = {
  ...
  description:
    ...
    "ทำขนาดตั้งแต่ 15 ถึง 20 ซม. วางได้ทั้งแนวตั้งและแนวนอน เลือกขนาดฐาน สกรีนฐาน และสกรีน 1-2 ด้านได้ " +
    "ราคารวมตัวงาน + ฐาน + กรอบการ์ดแล้ว 1-10 ชิ้นไม่มีขั้นต่ำ",
  highlights: [
    "ด้านหลังมีกรอบอะคริลิคใส (แนวตั้ง) สอดการ์ด 5.4 × 8.5 ซม. เปลี่ยนเองได้",
    "1-10 ชิ้น ราคารวมตัวสแตนดี้ + ฐาน + กรอบการ์ดครบ ไม่บวกค่าฐานเพิ่ม",
    "ทำขนาด 15-20 ซม. เลือกได้ทั้งแนวตั้งและแนวนอน ราคาเท่ากัน",
    "เลือกฐาน 6-7 ถึง 12 ซม. · ทรงกลม/สี่เหลี่ยม/ไดคัทตามทรง · สกรีนลายฐานได้",
  ...
  terms: [
    ...
    "ตั้งแต่ 11 ชิ้นขึ้นไป คิดค่าฐานเพิ่มตามขนาด: ไม่สกรีนฐาน 6-7 ซม. 15 บาท ถึง 12 ซม. 40 บาท · สกรีนลายฐาน 6-7 ซม. 25 บาท ถึง 12 ซม. 50 บาท",
...
async function uploadImages() {
  ...
  // คลิปงานจริง — อัปแยกเพราะเป็น mp4 (ต้องส่ง contentType ให้ถูก ไม่งั้นเบราว์เซอร์ไม่เล่นให้)
  const clip = await readFile(`${IMAGES_DIR.replace(/\/$/, "")}/clip-card.mp4`);
  const { error: clipErr } = await sb.storage
    .from("product-images")
    .upload(`products/${ID}/clip-card-${REV}.mp4`, clip, { contentType: "video/mp4", upsert: true });
  if (clipErr) throw new Error(`clip-card: ${clipErr.message}`);
  console.log(`⬆️  clip-card-${REV}.mp4 (${Math.round(clip.length / 1024)} KB)`);
}
