...
async function coatCards(coatFee, specialFee) {
  ...
  await coatCard(
    ...
    [`บวกเพิ่ม ${coatFee} บาท ต่อ 1 แผ่น A3 (ไม่ใช่ต่อชิ้น)`, "สีสดขึ้น เงาวาว กันน้ำ/รอยเปื้อนได้ดีกว่าไม่เคลือบ"]
  );
  await coatCard(
    "coat-matte",
    "เคลือบด้าน",
    ...
    [`บวกเพิ่ม ${coatFee} บาท ต่อ 1 แผ่น A3 (ไม่ใช่ต่อชิ้น)`, "นวลตา ไม่สะท้อนแสง ลายนิ้วมือไม่ติด"]
  );
  await coatCard(
    "coat-special",
    "เคลือบพิเศษ",
    ...
    [`บวกเพิ่ม ${specialFee} บาท ต่อ 1 แผ่น A3 — เช่น A5 4 ใบแรก ${specialFee} บาท · ใบที่ 5 ขึ้นแผ่นที่ 2 เป็น ${specialFee * 2} บาท`, "เลือกผิวฟิล์มย่อยได้ในกลุ่ม “เคลือบ” หลังเลือกแบบนี้"]
  );
}
...
/* ── 5. การ์ดกระดาษมาตรฐาน (วาดเอง) ─────────────────────────────── */

/**
 * เนื้อกระดาษพิเศษ 6 แบบใช้ "ภาพจริงจากคลังของร้าน" (products/texture-paper/*) ลิงก์ตรงใน apply.mjs
 * ที่นี่วาดเฉพาะการ์ด "กระดาษอาร์ต 300 แกรม" (กระดาษมาตรฐานของงานนี้) ที่คลังนั้นไม่มี — ให้ครบทุกตัวเลือกมีภาพ
 */
async function paperCard() {
  console.log("🖼  การ์ดกระดาษมาตรฐาน (วาดเอง — เนื้อกระดาษพิเศษใช้ภาพจริงจากคลัง texture-paper)");
  await saveSvg(
    "paper-art300",
    frame(`
      ${title("กระดาษอาร์ต 300 แกรม", "กระดาษมาตรฐานของงานนี้")}
      ${board(300, 260, 300, 400, { art: artwork(300, 260, 300, 400) })}
      ...
      ${foot(["อยากได้ผิวโฮโลแกรม / เมทัลลิก เลือกกระดาษเนื้อพิเศษได้", "กระดาษเนื้อพิเศษก็ไม่คิดค่ากระดาษเพิ่มเหมือนกัน"])}`)
  );
}
...
/**
 * ค่าบวกเพิ่มที่เขียนลงการ์ด — apply.mjs ทวนกับค่าจริงทุกครั้ง ไม่ตรง = หยุด
 * ⚠️ coat / special คิด "ต่อ 1 แผ่น A3" ไม่ใช่ต่อชิ้น (1 แผ่น A3 ได้ A5 4 ใบ · A6 8 ใบ · A7 16 ใบ)
 *    special = 30 ตามที่ร้านยืนยัน (หน้าเว็บ pricelists ยังเขียน 40 อยู่ — ดู apply.mjs)
 */
export const FEES = { coat: 10, special: 30, foil1: 40, foil2: 60, holo: 10 };
