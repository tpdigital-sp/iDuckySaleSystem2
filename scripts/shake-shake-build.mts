...
const hookOptions: ProductOption[] = [
  {
    ...clone(gateSrc),
    // 14 ก.ย. 69 จัดเป็นบรรทัด (ดู scripts/shake-shake-notes.mjs ที่แก้ของเดิมใน DB ให้ตรงกัน)
    note:
...
const OPTIONS: ProductOption[] = [
  {
    ...
    // สเปคโครงสร้างอยู่บนโน้ตกลุ่มแรก — ผู้ใช้สั่ง 25 ส.ค. 69 ให้เห็นตรงจุดเลือกตัวเลือกเลย ไม่ต้องเลื่อนหาแท็บ
    note:
      `**โครงสร้างกรอบ 3 ชั้นประกบกัน**: กรอบหน้าสกรีนลาย · ตัวกลาง หนา 1 มม. (ช่องให้ตัวน้อยขยับ) · กรอบหลัง หนา 1.5 มม. — ` +
  ...
  },
  {
    label: GROUP_SIZE,
    display: "multi",
    // 14 ก.ย. 69 เจ้าของร้านสั่งให้กระชับ (ดู scripts/shake-shake-notes.mjs ที่แก้ของเดิมใน DB ให้ตรงกัน)
    note:
      `กรอบเริ่มต้น **${BASE_CM} ซม.** (นับด้านยาวสุด ไดคัทตามทรงลายได้)\n` +
      `• ใหญ่ขึ้นได้ ติ๊กแล้วระบุจำนวน ซม. ที่เพิ่ม — **ซม. ละ ${OVERSIZE_BAHT} บาท** (กรอบ 8 ซม. = เพิ่ม 2 ซม.)`,
    choices: [
      {
        name: SIZE_CHOICE,
        extra: OVERSIZE_BAHT,
