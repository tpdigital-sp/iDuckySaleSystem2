...
const SHEET_HINT = "นับจุดไดคัทรวมทั้งแผ่น A3 (ดูวิธีนับในรูปประกอบแท็บ) — ส่วนที่เกินโควตาคิดจุดละ ฿0.50";
const SHEET_NOTE = (free) =>
  `โควตาจุดไดคัทที่รวมในราคา: ${free} จุดต่อ 1 แผ่น A3 — ส่วนที่เกินคิดจุดละ ฿${DOT_FEE.toFixed(2)} ต่อแผ่น` +
  ` (สั่ง 25 แผ่น A3 ขึ้นไปต่อ 1 ลาย ฟรีค่าจุด — แจ้งแอดมินปรับให้)`;
const HINT = "นับจุดไดคัทของลาย 1 ชิ้น (ดูวิธีนับในแท็บ “ข้อควรทราบ”) — เกินโควตาของขนาดที่เลือกคิดจุดละ ฿0.50";
const NOTE =
  `โควตาจุดไดคัทที่รวมในราคา: ${quotaText} — ส่วนที่เกินคิดจุดละ ฿${DOT_FEE.toFixed(2)} ต่อแผ่น A3` +
  ` (สั่ง 25 แผ่น A3 ขึ้นไปต่อ 1 ลาย ฟรีค่าจุด — แจ้งแอดมินปรับให้)`;
...
for (const id of IDS) {
  ...
  for (const plan of plans) {
    ...
    sizeGroup.note =
      `โควตาจุดไดคัทที่รวมในราคา: ${quotaText} — เกินคิดจุดละ ฿${DOT_FEE.toFixed(2)} (กรอกจำนวนจุดที่ช่องด้านล่าง)` +
      ` · วางลายห่างกันอย่างน้อย 2 มม. · ดูวิธีนับจุดในแท็บ “ข้อควรทราบ”`;
  }
