...
/* ── 4. ตัดเต็มหลา: ต่อผืนยาวสุด 360 ซม. (4 หลา) ─────────────────── */
const cut = opt(G_CUT);
...
cut.note =
  ...
  `ต่อผืนยาวสูงสุด ${MAX_LEN} ซม. (${MAX_YARDS} หลา) · **ตัดแบ่งตามขนาด**: ระบุขนาดชิ้นงานแล้วระบบคำนวณให้อัตโนมัติ — ` +
...
edge.note =
  ...
  `**ตัดเต็มหลา** คิดที่ขั้น 150 ซม. ต่อผืน · **ตัดแบ่ง** ระบบคิดตามขนาดชิ้น × จำนวนชิ้นต่อหลาให้อัตโนมัติ · ` +
...
const TERM_HEADS = ["*ตัดเต็มหลา หน้ากว้างสูงสุด", "*เย็บขอบเลือกสีไหมได้", "*งานสั่งทำพิเศษนอกเหนือจาก"];
d.terms = [
  ...String(d.terms ?? "")
    .split("\n")
