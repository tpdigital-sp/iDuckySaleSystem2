...
/** ค่าเคลือบเงาด้านหน้า — คิดต่อแผ่น A3 (ไม่ได้รวมในราคากระดาษแล้ว) */
const GLOSS_FEE = 10;
...
const front = {
  label: FRONT_LABEL,
  // ชุดเดียวกับกลุ่มกระดาษพิเศษของ cup-sleeve — แต่ร้านคิดค่าเคลือบเงาเพิ่มแผ่นละ 10 บาท (แจ้ง 21 ส.ค. 69)
  choices: [{ name: "เคลือบเงา", extra: GLOSS_FEE }, { name: "ไม่เคลือบ" }],
...
const swap = (s) =>
  typeof s !== "string"
    ...
    : s
        ...
        .replace(
          "กระดาษโฮโลแกรม และกระดาษสีเงิน/สีทอง ผิวเงา เคลือบเงาด้านหน้า ฟรี (รวมในราคาแล้ว) — ด้านหลังเคลือบไม่ได้",
          `กระดาษโฮโลแกรม และกระดาษสีเงิน/สีทอง ผิวเงา เคลือบเงาด้านหน้าได้ +${GLOSS_FEE} บาท/แผ่น — ด้านหลังเคลือบไม่ได้`
        )
        .replace(
          "เคลือบเงาด้านหน้าให้ฟรี (รวมในราคาแล้ว) — เคลือบได้เฉพาะด้านหน้า ด้านหลังเคลือบไม่ได้",
          `เคลือบเงาด้านหน้าได้ บวกเพิ่มแผ่นละ ${GLOSS_FEE} บาท — เคลือบได้เฉพาะด้านหน้า ด้านหลังเคลือบไม่ได้`
        )
        .replace(
          "• เคลือบเงาด้านหน้า ฟรี เฉพาะโฮโลแกรมและสีเงิน/สีทอง ผิวเงา (รวมในราคาแล้ว) — ด้านหลังเคลือบไม่ได้",
          `• เคลือบเงาด้านหน้า +${GLOSS_FEE} บาท/แผ่น เฉพาะโฮโลแกรมและสีเงิน/สีทอง ผิวเงา — ด้านหลังเคลือบไม่ได้`
        )
        .replace(
          "SeaSand และ Rainbow — เคลือบเงา ฟรี (พิมพ์รองสีขาวเพิ่ม +20 บาท/แผ่น)",
          `SeaSand และ Rainbow — เคลือบเงา +${GLOSS_FEE} บาท/แผ่น (พิมพ์รองสีขาวเพิ่ม +20 บาท/แผ่น)`
        )
        .replace(
          'เฉพาะ "ผิวเงา" เคลือบเงา ฟรี · ผิวด้านเคลือบไม่ได้',
          `เฉพาะ "ผิวเงา" เคลือบเงาได้ +${GLOSS_FEE} บาท/แผ่น · ผิวด้านเคลือบไม่ได้`
        );
