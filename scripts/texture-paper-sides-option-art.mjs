#!/usr/bin/env node
/**
 * กระดาษ Texture Paper (texture-paper) — ภาพประกอบกลุ่ม "จำนวนด้านที่พิมพ์" + แสดงเป็นการ์ด
 *
 *   node scripts/texture-paper-sides-option-art.mjs           (วาดลง .cache/texture-paper/upload ดูก่อน)
 *   node scripts/texture-paper-sides-option-art.mjs --write   (+ อัปโหลด storage + เขียน options + อ่านกลับเทียบ)
 *
 * ดีไซน์: แผ่นเดียวกันสองมุม "ด้านหน้า | ด้านหลัง" มีลูกศรพลิกคั่นกลาง + ป้ายเลข 1/2 กลางภาพ
 *   ต่างจากโปสเตอร์ตรงที่นี่คือ "กระดาษผิวพิเศษ" — ด้านหลังที่ไม่พิมพ์จึงไม่ใช่กระดาษขาว
 *   แต่เป็นผิวมุก/เมทัลลิกเปล่า ๆ (มีประกายแต่ไม่มีลาย) ให้ตรงกับของจริง
 *   จุดต่างที่อ่านออกตั้งแต่ปุ่ม 62px = "แผ่นขวาเป็นผิวเงินเปล่า vs แผ่นขวามีลายสี" + เลข 1/2 กลางภาพ
 *   (ภาพจัตุรัส 900×900 เท่าช่อง object-cover ของปุ่ม จึงเห็นเต็มใบ ไม่ถูกครอป)
 *
 * ⚠️ ไม่ใส่ตัวเลขค่าบริการลงในภาพ — +฿ ของ "พิมพ์ 2 ด้าน" อยู่ใน choice.extra (หน้าเว็บโชว์ป้ายเอง)
 * ⚠️ ห้ามแก้ชื่อตัวเลือก — กลุ่ม "เคลือบ (ด้านหลัง)" ใช้ showWhen อ้าง "พิมพ์ 2 ด้าน" อยู่
...
/** แผ่นกระดาษย่อ — อัตราส่วน A (29.7 : 42) เหมือนแผ่น A3 ก่อนตัด */
const SW = 206;
...
const PICKS = [
  {
    ...
    back: false,
    title: "พิมพ์ 1 ด้าน",
    sub: "พิมพ์ลายเฉพาะด้านหน้า · ด้านหลังเป็นผิวกระดาษเปล่า",
    foot: "ส่งไฟล์ลายมา 1 ไฟล์",
    desc: "พิมพ์ลายด้านหน้าอย่างเดียว ด้านหลังเป็นผิวกระดาษเปล่า · ส่งไฟล์ลายมา 1 ไฟล์",
  },
  {
    name: "พิมพ์ 2 ด้าน",
...
/** ประกายผิวกระดาษพิเศษ — แถบแสงเฉียงพาดแผ่น (ใช้ทั้งแผ่นที่พิมพ์และไม่พิมพ์) */
const shimmer = (x, y, op = 1) => `
  <g opacity="${op}">
    <path d="M ${x - 20} ${y + SH * 0.34} L ${x + SW * 0.62} ${y - 20} L ${x + SW * 0.86} ${y - 20} L ${x - 20} ${y + SH * 0.62} Z" fill="#ffffff" opacity="0.34"/>
    <path d="M ${x + SW * 0.34} ${y + SH + 20} L ${x + SW + 20} ${y + SH * 0.46} L ${x + SW + 20} ${y + SH * 0.66} L ${x + SW * 0.7} ${y + SH + 20} Z" fill="#ffffff" opacity="0.22"/>
  </g>`;

/** ลายด้านหน้า — มาสคอตบน หัวเรื่อง+บรรทัดข้อความล่าง */
const frontArt = (x, y) => {
  const mh = SH * 0.46;
...
/** แผ่นกระดาษหนึ่งใบ — พิมพ์แล้ว (ลายบนผิวพิเศษ) หรือเปล่า (ผิวเมทัลลิกล้วน) */
const sheet = (x, printed, grad, art) => `
  <rect x="${x + 6}" y="${SY + 10}" width="${SW}" height="${SH}" rx="4" fill="#0f172a" opacity="0.12"/>
  <rect x="${x}" y="${SY}" width="${SW}" height="${SH}" rx="4" fill="url(#${printed ? grad : "bare"})" stroke="#94a3b8" stroke-width="2"/>
  <g clip-path="url(#clip-${grad})">
    ${printed ? art : ""}
    ${shimmer(x, SY, printed ? 0.55 : 1)}
  </g>
  ${printed ? "" : `<text x="${x + SW / 2}" y="${SY + SH / 2 + 10}" font-family="${TH}" font-size="25" text-anchor="middle" fill="#94a3b8">ผิวกระดาษเปล่า</text>`}`;
...
function art(p) {
  ...
  <defs>
    ...
    <!-- ผิวกระดาษพิเศษที่ยังไม่พิมพ์ — ประกายมุก/เมทัลลิก -->
  ...
  <text x="${W / 2}" y="${H - 56}" font-family="${TH}" font-size="22" text-anchor="middle" fill="${SUB}">กระดาษผิวพิเศษแผ่นเดียวกัน · เลือกได้ว่าจะพิมพ์กี่ด้าน</text>
</svg>`;
}
...
/* กันเผลอ: ค่าบริการ 2 ด้าน + กลุ่มเคลือบด้านหลังที่อ้างชื่อตัวเลือกนี้ ต้องอยู่ครบเหมือนเดิม */
const two = g.choices.find((c) => c.name === "พิมพ์ 2 ด้าน");
if (!(two?.extra > 0)) { console.error("ค่าบริการพิมพ์ 2 ด้านหาย!", two); process.exit(1); }
const backCoat = back.data.options.find((o) => o.showWhen?.label === GROUP);
if (!backCoat?.showWhen?.choices?.includes("พิมพ์ 2 ด้าน")) { console.error("กลุ่มที่อ้าง showWhen จำนวนด้านหาย!", backCoat?.label); process.exit(1); }
...
console.log(`✓ กลุ่ม "${GROUP}" เป็นการ์ด + ภาพ ${built.length} ใบ · 2 ด้าน +฿${two.extra} · showWhen "${backCoat.label}" ยังอยู่ · savedAt =`, back.data.savedAt);
