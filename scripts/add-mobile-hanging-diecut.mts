...
const PET_CLEAR = "PET ใส";
/** งาน PET ใส เลือกได้ว่าจะพิมพ์รองพื้นขาวใต้ลายไหม — รองขาว บวกเพิ่มเซ็ตละ 20 บาท (ผู้ใช้สั่ง 25 ส.ค. 69) */
const WHITE_BASE_LABEL = "รองพื้นขาว (เฉพาะ PET ใส)";
const WHITE_BASE_FEE = 20;
...
/** ลำดับกลุ่ม: ขนาด (สูง → กว้าง) → วัสดุ PET → รองพื้นขาว (ผู้ใช้จัดลำดับ 25 ส.ค. 69) */
const OPTIONS: ProductOption[] = [
  /**
   * ลูกค้าระบุขนาดที่ต้องการทำได้ (ผู้ใช้สั่ง 25 ส.ค. 69) — เงื่อนไขจากเว็บ: ไม่เกิน สูง 10 × กว้าง 6 cm
   * ช่องกรอกแบบ standardInput = ข้อมูลประกอบของงานปกติ ราคายังคิดตามตารางเดิม (ไม่ใช่งานตีราคา)
   * min/max กันลูกค้าพิมพ์ขนาดที่ผลิตไม่ได้ตั้งแต่หน้าสินค้า
   */
  {
    label: "ขนาดชิ้นงาน (สูง)",
    choices: [],
    display: "input",
    standardInput: true,
    input: { kind: "number", unit: "ซม.", min: 1, max: 10, placeholder: "เช่น 8", hint: "ทำได้ไม่เกินความสูง 10 ซม. (ไม่รวมที่ห้อย 1.5 ซม.)" },
  },
  {
    label: "ขนาดชิ้นงาน (กว้าง)",
    choices: [],
    display: "input",
    standardInput: true,
    input: { kind: "number", unit: "ซม.", min: 1, max: 6, placeholder: "เช่น 5", hint: "ทำได้ไม่เกินความกว้าง 6 ซม." },
  },
  {
    label: PET_LABEL,
    display: "pills",
    choices: [
      // piecesPerUnit = โชว์กล่องสรุป "ได้ 5 ชิ้น ต่อ 1 เซ็ต · สั่ง N เซ็ต = ได้ N×5 ชิ้น" (ผู้ใช้สั่ง 25 ส.ค. 69)
      { name: "PET สีขาว", perUnit: PER_SET, piecesPerUnit: PER_SET, imageSrc: art["pet-white"] },
      { name: PET_CLEAR, perUnit: PER_SET, piecesPerUnit: PER_SET, imageSrc: art["pet-clear"] },
    ],
  },
  {
    // โผล่เฉพาะตอนเลือก PET ใส — ไม่รองขาว = ลายโปร่งแสงตามเนื้อ PET · รองขาว = สีทึบสด +20/เซ็ต
    label: WHITE_BASE_LABEL,
    display: "pills",
    note: "ไม่รองขาว = ลายโปร่งแสง มองทะลุตามเนื้อ PET ใส · รองขาว = พิมพ์รองพื้นขาวใต้ลาย สีทึบสดเหมือนงานขาว",
    showWhen: { label: PET_LABEL, choices: [PET_CLEAR] },
    choices: [
      { name: "ไม่รองขาว", badge: "ฟรี" },
      { name: "รองขาว", extra: WHITE_BASE_FEE },
    ],
  },
];
...
const product: Product = {
  ...
  terms: [
    ...
    `*งาน PET ใส เลือกได้ว่า ไม่รองขาว (ลายโปร่งแสง) หรือ รองขาว (สีทึบสด) — รองขาว บวกเพิ่มเซ็ตละ ${WHITE_BASE_FEE} บาท`,
    "*ระบุขนาดที่ต้องการทำได้ — ไดคัทตัดตามทรงลาย ทำขนาดไม่เกิน สูง 10 cm × กว้าง 6 cm",
  ...
  tabs: [
    {
      ...
      text: [
        `• ขายเป็นเซ็ต 1 เซ็ตได้ ${PER_SET} ชิ้น (เช่น สั่ง 10 เซ็ต จะได้รับ ${10 * PER_SET} ชิ้น) · วัสดุเป็น PET ( สีขาว | ใส ) หนา 250 ไมครอน`,
        `• งาน PET ใส เลือกได้ว่า ไม่รองขาว (ลายโปร่งแสง มองทะลุได้) หรือ รองขาว (พิมพ์รองพื้นขาวใต้ลาย สีทึบสด) — รองขาว บวกเพิ่มเซ็ตละ ${WHITE_BASE_FEE} บาท`,
        "• ระบุขนาดที่ต้องการทำได้ — ไดคัทตัดตามทรงลาย ทำขนาดไม่เกิน สูง 10 cm × กว้าง 6 cm · ที่ห้อยยาว 1.5 cm (ช่องห้อยประมาณ 8 mm)",
        "• งานพิมพ์ Digital Printing",
        "• ใส่หลังเคส ผ่านรูของช่องเสียบสายชาร์จ — รูเคสต้องมีขนาดมากกว่า 1.1 cm ถึงจะใส่ได้",
  ...
  seo: {
    ...
    faqs: [
      ...
      },
      {
        ...
        a: `PET สีขาวพื้นทึบ ลายเด่นชัดเหมือนงานการ์ด ส่วน PET ใสมองทะลุได้ ให้ลุคโปร่ง ๆ เห็นสีเคส/ตัวเครื่องลอดออกมา — ความหนา 250 ไมครอนเท่ากันทั้งสองแบบ ราคาเท่ากัน · งาน PET ใส เลือกเพิ่มได้ว่าจะรองขาวไหม (รองขาว = พิมพ์รองพื้นขาวใต้ลาย สีทึบสด บวกเพิ่มเซ็ตละ ${WHITE_BASE_FEE} บาท · ไม่รองขาว = ลายโปร่งแสง ไม่คิดเพิ่ม)`,
      },
      {
        q: "สั่ง 1 เซ็ต ได้กี่ชิ้น? กำหนดขนาดเองได้ไหม?",
        a: `1 เซ็ตได้ ${PER_SET} ชิ้น เช่น สั่ง 10 เซ็ต จะได้รับ ${10 * PER_SET} ชิ้น — ระบุขนาดที่ต้องการทำได้ตอนสั่ง แต่ไม่เกินความ สูง 10 ซม. × กว้าง 6 ซม.`,
      },
      {
