...
async function paperCards() {
  ...
  const svg = frame(`
    ${title("กระดาษพิเศษ", "เนื้อกระดาษนอกเหนือจากอาร์ตมัน")}
    ${cupWithSleeve(268, 200, 380, { sleeveFill: "#f1e9dc" })}
    ${flatSleeve(640, 700, 230, 155, { fill: "#f1e9dc" })}
    <text x="640" y="742" font-family="${TH}" font-size="22" text-anchor="middle" fill="${SUB}">ตัวปลอกก่อนสวม (แบบแบน)</text>
    ${["คราฟท์ · ลูกฟูก · กระดาษมีลาย", "เนื้อสัมผัสพิเศษตามที่ลูกค้าเลือก"]
      .map((l, i) => `<text x="640" y="${268 + i * 44}" font-family="${TH}" font-size="26" text-anchor="middle" fill="${INK}">${esc(l)}</text>`)
      .join("")}
    <text x="640" y="360" font-family="${TH}" font-size="22" text-anchor="middle" fill="${SUB}">แจ้งชนิดกระดาษที่ต้องการกับแอดมิน</text>
    ${foot(["ราคาคิดตามชนิดกระดาษที่เลือก", "แอดมินตีราคาให้ก่อนเริ่มผลิตทุกงาน"])}`);
  await saveSvg("paper-special", svg);
}
