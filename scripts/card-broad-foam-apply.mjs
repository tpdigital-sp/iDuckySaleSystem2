...
d.rules = [
  ...
  /**
   * งานฟอยล์ทำร่วมกับเคลือบลามิเนตไม่ได้ (ผู้ใช้ยืนยัน 21 ส.ค. 69) —
   * เลือกฟอยล์เมื่อไหร่ กลุ่มเคลือบลามิเนตเหลือ "ไม่เคลือบ" อย่างเดียว
   * ใส่ทางเดียวพอ — อยากกลับไปเคลือบลามิเนตให้เปลี่ยนฟอยล์เป็น "ไม่ปั๊มฟอยล์" ก่อน
   */
  {
    when: {
      label: FOIL_LABEL,
      choice: FOILS.filter((f) => f.fee).map((f) => f.name)[0],
      choices: FOILS.filter((f) => f.fee).map((f) => f.name),
    },
...
d.terms = [
  ...
  `งานฟอยล์ทำร่วมกับการเคลือบลามิเนตไม่ได้ — เลือกได้อย่างใดอย่างหนึ่ง (เลือกฟอยล์แล้ว หน้าสินค้าจะล็อกเคลือบลามิเนตเป็น "ไม่เคลือบ" ให้เอง)`,
...
d.tabs = [
  {
    ...
    text: [
      ...
      "• เลือกฟอยล์แล้ว เคลือบลามิเนตจะถูกล็อกเป็น “ไม่เคลือบ” (ทำร่วมกันไม่ได้)",
    ].join("\n"),
