 ...
 *    สคริปต์จึง "หารลงต่อใบ" ตามขนาดที่ลูกค้าเลือก แล้วบวกเข้าไปในช่องตารางราคาเลย
 *    → ค่าเคลือบ/ฟอยล์เป็น "แกนของตารางราคา" ไม่ใช่ +฿ ต่อชิ้นแบบคงที่ เพราะค่าต่อใบต่างกันตามขนาด
 *
 * ⚠️ ชิ้นงานหนึ่งใบราคาอาจมีทศนิยม (เช่น A7 เคลือบเงา = 70 + 10÷16 = ฿70.63) — ตั้งใจให้เป็นแบบนั้น
 *    สั่งครบแผ่น (16 ใบ) ยอดรวมจะเท่ากับค่าเคลือบ 1 แผ่นพอดี
 *
...
for (const s of SIZES)
  for (const c of COATS)
    for (const f of FOILS)
      cells[`${s.name}│${c.name}│${f.name}`] = base[s.name].map((p) => round2(p + (c.fee + f.fee) / s.perSheet));

const PRICING = {
  unit: "ชิ้น",
  driverLabels: [SIZE_LABEL, COAT_LABEL, FOIL_LABEL],
  tiers,
  cells,
};
...
d.highlights = [
  ...
  `เคลือบเงา/ด้าน +${COAT_FEE} บาท · เคลือบพิเศษ +${SPECIAL_FEE} บาท · ปั๊มฟอยล์ +${FOIL_1}-${FOIL_2} บาท (คิดต่อแผ่น A3 หารลงต่อใบให้แล้ว)`,
...
d.options = [
  ...
  {
    label: COAT_LABEL,
    choices: COATS.map((c) => ({ name: c.name, ...(c.popular ? { popular: true } : {}), imageSrc: art[c.art] })),
  },
  {
    label: FILM_LABEL,
    display: "pills",
  ...
  {
    label: FOIL_LABEL,
    choices: FOILS.map((f) => ({ name: f.name, ...(f.popular ? { popular: true } : {}), imageSrc: art[f.art] })),
  },
  {
...
d.tabs = [
  {
    ...
    text: [
      ...
      "::ราคาต่อชิ้น (ยังไม่รวมเคลือบ/ฟอยล์)::",
  ...
  faqs: [
    {
      ...
      a: `คิดต่อชิ้นตามขนาดและจำนวน — ${headSizes.map((n) => `${n} ชิ้นละ ${base[n][0]}-${base[n].at(-1)} บาท`).join(" · ")} (ยังไม่รวมเคลือบ/ฟอยล์)`,
    },
    {
