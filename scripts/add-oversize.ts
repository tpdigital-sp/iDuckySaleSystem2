 ...
 *
 * ราคาจากเว็บ (4 แท็บของบล็อกนั้น) → 3 เรทราคา + แกน "จำนวนจุดที่สกรีน"
 ...
 *      — จุดที่ 2 เป็นต้นไปคิดเพิ่มตามขนาด/ช่วงจำนวน (30/45/95 บาท ลดหลั่นถึง 15/28/50)
 ...
 * base = ราคาเสื้อ 1 จุด ต่อขนาด · feeTier = ช่วงจำนวนจุดที่ใช้กับช่วงจำนวนตัวแต่ละช่วง
 * (สั่ง 11-29 ตัว สกรีนตัวละ 1 จุดเพิ่ม = 11-29 จุด จึงใช้ราคาช่วงเดียวกัน)
 */
function matrixWithPoints(tiers: PriceTier[], base: Record<string, number[]>, feeTier: number[]) {
  const cells: Record<string, number[]> = {};
  for (const size of SIZES) {
    for (let p = 0; p < POINTS.length; p++) {
      cells[`${size}│${POINTS[p]}`] = base[size].map(
        (price, ti) => price + p * POINT_FEE[size][feeTier[ti]]
      );
    }
  }
  return { unit: UNIT, driverLabels: [SIZE_LABEL, POINT_LABEL], tiers, cells };
}
...
const product: Product = {
  ...
  description:
    ...
    "เลือกขนาดสกรีนได้ตั้งแต่ไม่เกิน 5 นิ้ว จนถึง A4/A3 และสกรีนได้มากกว่า 1 จุด (อก + หลัง + แขน) " +
  ...
  options: [
    ...
    },
    {
      label: POINT_LABEL,
      showWhen: { label: RATE_LABEL, choices: [RATE_DTF, RATE_FLEX] },
      choices: [{ name: P1 }, { name: P2 }, { name: P3 }],
    },
    {
  ...
  terms: [
    ...
    "สกรีนจุดที่ 2 เป็นต้นไป คิดเพิ่มต่อจุดตามขนาด — ไม่เกิน 5 นิ้ว 30-15 บาท · A5 45-28 บาท · A4/A3 95-50 บาท (ยิ่งจำนวนจุดเยอะยิ่งถูก)",
