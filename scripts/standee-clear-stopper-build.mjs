 ...
 *             + ค่าแผ่นบนตามขนาด (20 คงที่ทุกช่วงจำนวน + เซนละ 10 จาก 2 ซม.) + ค่าสกรีนของแผ่นบนตามขนาดแผ่นบน
 *   ⚠️ แผ่นบน "ไม่ลดตามจำนวน" — ต่างจากพวงกุญแจ+จุกใส ที่คิดแบบติ่งห้อย 20/15/12
 *      (ผู้ใช้ยืนยัน 2 ก.ย. 69 พร้อมใบเสนอราคาจริง: 11 เซ็ต · ตัว 6cm 69 + แผ่นบน 2cm 20 + ฐาน 3cm 10 + จุกใส 10 = 109)
 *   ค่าฐานคิดแยกที่กลุ่ม "ขนาดฐาน" (extraFromQty 11 · extraBelow ช่วงปลีก) เหมือน standy ทุกประการ
 *
 * ⛔ สคริปต์นี้เป็นตัว "สร้างครั้งแรก" — ล้าสมัยแล้ว ถ้าจะรันซ้ำต้องรัน
 *    scripts/standee-clear-stopper-repricing.mjs ต่อทันที ไม่งั้นจะย้อนกลับไปเป็นของเก่า:
 *      • ไม่มีเรทที่ 2 (สั่งแบบไม่คละดีเทล) — ดู scripts/standee-clear-stopper-rate2.mjs
 *      • กลุ่มฐานที่โคลนจาก standy มี extraFromQty 11 = ช่วงปลีกไม่คิดค่าฐาน
 *        (ผู้ใช้สั่ง 2 ก.ย. 69 ให้คิดค่าฐานตามตารางทุกช่วงจำนวน)
 *
...
/** ราคาแผ่นบนต่อชิ้น ตามขนาด — คงที่ทุกช่วงจำนวน (2 ซม. = 20 · 3 ซม. = 30 … 10 ซม. = 100) */
...
for (const [b, t, s] of [
  ...
  const check = `${SHEET[b][1]} + ${/2 ด้าน/.test(s) ? SCREEN_FEE[b] : 0} + ${STOPPER_FEE} + ${topPlatePrice(t)} + ${/2 ด้าน/.test(s) ? SCREEN_FEE[t] : 0}`;
