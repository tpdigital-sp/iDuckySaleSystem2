 ...
 *
 * ขนาด 3 แบบมาจาก "คลังเทมเพลต" ของร้านเอง (หมวด cup sleeve — ไฟล์ไดคัท .ai 3 ไฟล์)
 *   หน้าเว็บตารางราคาบอกแค่ "ปรับขนาดได้ 3 ระดับ" ไม่ได้ลงตัวเลข ขนาดจริงจึงยึดตามไฟล์ไดคัทที่ร้านมี
 *   และผูก templateIds ไว้ให้ลูกค้าโหลดไฟล์ไปวางลายได้เลย
 *
...
import { SIZES } from "./cup-sleeve-art.mjs";
...
d.description =
  ...
  `เลือกได้ 3 ขนาด (${SIZES.map((s) => s.name).join(" · ")}) ปลายปลอกล็อกปรับความกว้างได้อีก 3 ระดับ ` +
...
d.highlights = [
  ...
  `เลือกได้ 3 ขนาด (${SIZES.map((s) => s.name).join(" · ")}) ราคาเท่ากัน + ล็อกปรับความกว้างได้ 3 ระดับ`,
  `เคลือบเงา / ด้าน +฿${COAT_FEE} ต่อด้าน · เคลือบพิเศษ +฿${SPECIAL_FEE} ต่อด้าน`,
];

d.templateIds = SIZES.map((s) => s.tpl);

d.options = [
  {
    label: SIZE_LABEL,
    // ไม่ติดป้าย "นิยม" ให้ขนาดไหน — ยังไม่มีข้อมูลว่าลูกค้าสั่งขนาดไหนบ่อยสุด
    choices: SIZES.map((s) => ({ name: `${s.name} (${s.note.split(" — ")[0]})`, imageSrc: art[s.key] })),
  },
  {
    label: PAPER_LABEL,
    stockBearing: true,
...
d.terms = [
  ...
  `1 เซ็ตใช้ได้ 1 ลาย — อยากได้หลายลาย เพิ่มจำนวนเซ็ตตามจำนวนลาย`,
  ...
  `มี 3 ขนาดให้เลือก (${SIZES.map((s) => s.name).join(" · ")}) ราคาเท่ากันทุกขนาด — เป็นทรงมาตรฐานของร้าน ไม่ได้ตัดตามแก้วเฉพาะรุ่น`,
  "ปลายปลอกมีรอยล็อก 3 ตำแหน่ง ปรับความกว้างได้อีกตามขนาดแก้ว",
...
const STD_TABS = [
  {
    ...
    text: [
      ...
      "• เลือกขนาด → ชนิดกระดาษ → การเคลือบ → ใส่จำนวนเซ็ต",
      ...
      "• ระบุรายละเอียด: ขนาดปลอก · ชนิดกระดาษ · การเคลือบ · จำนวนเซ็ต · วันที่ใช้งาน (ถ้ามี)",
    ].join("\n"),
  },
  {
    title: "การเตรียมไฟล์",
    text: [
      "• โหลดไฟล์เทมเพลตไดคัทของขนาดที่เลือกไปวางลายได้เลย (ไฟล์ .ai ที่หน้าสินค้า)",
      "• ไฟล์นามสกุล .Ai .Psd .PNG พื้นหลังใส ความละเอียดสูง",
      "• ตัดตกจากขนาดงานจริงด้านละ 3mm · ไม่ควรวางงานชิดขอบหรือมีเส้นขอบ (ตัดคลาดเคลื่อนได้ 0.5-2mm)",
...
d.tabs = [
  {
    ...
    text: [
      ...
      "::ขนาด (กางแบน)::",
      `• ${SIZES.map((s) => `${s.name} — ${s.note}`).join("\n• ")}`,
      "• ทั้ง 3 ขนาดราคาเท่ากัน · ปลายปลอกมีรอยล็อก 3 ตำแหน่ง ปรับความกว้างได้อีก",
      "• มีไฟล์เทมเพลตไดคัทให้โหลดครบทั้ง 3 ขนาด",
      "::วัสดุ::",
    ...
    images: [art["set-of-6"], art["size-compare"], art["size-3-levels"]],
    imagePos: "bottom",
    imageSize: "md",
  },
  {
    title: "ข้อควรทราบ",
    text: [
      `• 1 เซ็ตใช้ได้ 1 ลาย — สั่ง 2 เซ็ตถึงจะคละได้ 2 ลาย (ลายละ ${PER_SET} ชิ้น)`,
...
d.seo = {
  ...
  description:
    ...
    `เลือกได้ 3 ขนาด ปรับความกว้างได้ 3 ระดับ เคลือบเงา ด้าน กลิตเตอร์ โฮโลแกรมได้ ไม่มีขั้นต่ำ`,
  keywords: [
    "cup sleeve",
    "ที่ครอบแก้ว",
    "ปลอกแก้วกระดาษ",
  ...
  faqs: [
    ...
    {
      q: "มีขนาดไหนให้เลือกบ้าง?",
      a: `เลือกได้ 3 ขนาด (กางแบน) — ${SIZES.map((s) => `${s.name} ${s.note.split(" — ")[0]}`).join(" · ")} ราคาเท่ากันทุกขนาด และปลายปลอกมีรอยล็อก 3 ตำแหน่ง ปรับความกว้างได้อีก`,
    },
    {
