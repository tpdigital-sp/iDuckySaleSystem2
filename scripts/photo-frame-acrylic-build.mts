 ...
 *   • ค่าฐานเป็นราคาเดียวทุกจำนวน (เจ้าของร้านเคาะอีกรอบ 15 ก.ย. 69 — กลับไปใช้กติการอบแรก):
 *     "งานสแตนดี้ ปลีกหรือส่งก็บวกค่าฐานเหมือนกันตามตารางค่าฐานสแตนดี้เลย"
 *     = extra ตัวเดียวตามตารางร้าน 3-5=10 · 6-7=15 · 8=20 · ไม่มี extraBelow / extraFromQty อีกแล้ว
 */
...
console.log(
  `   ${"ค่าฐาน (ปลีก = ส่ง)".padEnd(24)} ${BASE_SIZES.map((n) => `${baseName(n)}:${baseFee(n)}`).join(" ")}` +
...
gType.display = "cards";
gType.note = "พวงกุญแจ = เจาะรู + โซ่ไข่ปลา · สแตนดี้ = ตัวงานราคาเท่ากัน แต่คิดค่าฐานเพิ่มตามขนาดฐาน (คิดเท่ากันทั้งปลีกและส่ง)";
...
setChoice(gType, "สแตนดี้", {
  desc: `เสียบฐานอะคริลิค ตั้งโชว์บนโต๊ะ — เลือกขนาดฐานและจะสกรีนลายฐานด้วยก็ได้ (ค่าฐาน ${baseText} ต่ออัน · ปลีกหรือส่งเท่ากัน)`,
...
// ค่าฐานราคาเดียวทุกจำนวน (เจ้าของร้านเคาะ 15 ก.ย. 69) — ไม่มี extraFromQty/extraBelow
delete gBaseSize.extraFromQty;
gBaseSize.note = `ค่าฐานคิดเท่ากันทั้งราคาปลีกและราคาส่ง ตามตารางค่าฐานสแตนดี้ของร้าน (${bandText})`;
...
gBaseSize.choices = BASE_SIZES.map((n) => ({
  name: baseName(n),
  desc: `${BASE_DESC[n]} · ปลีกหรือส่ง +฿${baseFee(n)} เท่ากัน`,
...
gBase.display = "cards";
delete gBase.extraFromQty; // สกรีนลายฐาน +10 คิดเท่ากันทุกช่วงจำนวน เหมือนค่าฐานเอง
...
const DETAIL = [
  ...
  `• แบบสแตนดี้ คิดค่าฐานอะคริลิคเพิ่มตามขนาดฐาน — คิดเท่ากันทั้งราคาปลีกและราคาส่ง` +
    ` ตามตารางค่าฐานสแตนดี้ของร้าน (${bandText.replaceAll("฿", "")} บาท)` +
  ...
  "• จำนวน 11 อันขึ้นไปคละลายได้ ขั้นต่ำลายละ 5 อัน — สั่ง 11 อันได้ฟรี 2 ลาย เกินจากนั้นคิดเพิ่มลายละ 5 บาท",
].join("\n");
d.tabs = [
