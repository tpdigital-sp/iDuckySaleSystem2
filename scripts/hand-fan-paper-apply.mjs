...
if (MAX_W !== MAX_H || BIGGEST.mm !== MAX_W * 10)
  throw new Error(
    `เว็บบอกขนาดใหญ่สุด ${MAX_W}x${MAX_H} cm แต่ช่วงขนาดใหญ่สุดในไฟล์ art คือ ${BIGGEST.mm / 10} ซม. — แก้ตาราง SIZES ก่อน`
  );
...
d.options = [
  {
     ...
     */
    label: SIZE_LABEL,
    note: `ไดคัทตามทรงที่ออกแบบมา ขนาดไม่เกิน ${MAX_W} × ${MAX_H} ซม. · ราคาต่อ${UNIT}เท่ากันทุกช่วง — ช่วงขนาดใช้คิดค่าเคลือบต่อ${SHEET_UNIT}`,
    choices: SIZES.map((s) => ({
      name: s.name,
