 ...
 *       ตัวหารจึงมาจากผังวางบนแผ่น A3 ในกลุ่ม "ขนาดพัด" (ตาราง SIZES ของ hand-fan-paper-art.mjs)
 *       ตอนนี้ร้านเปิดช่วงเดียว = ขนาดใหญ่สุด 20×20 ซม. → 2 อัน/แผ่น A3 เสมอ
 *       กลุ่มนี้ไม่เปลี่ยนราคาต่ออัน — ราคาต่ออันบนเว็บเท่ากันทุกขนาดที่ไม่เกิน 20×20 ซม.
 *
 * ⚠️ หน้าเว็บบล็อกนี้เขียนไว้แค่ "เคลือบ เงา" — แต่ร้านยืนยัน 21 ส.ค. 69 ว่ารับ "เคลือบด้าน" ด้วย
 *    และคิดราคาเท่ากับเคลือบเงา (เหมือน CUP SLEEVE ที่หน้าเว็บเขียน "เคลือบด้าน / เงา" ไว้ในบรรทัดเดียว)
 *    ค่าเคลือบด้านจึงอ้างเลขตัวเดียวกับเคลือบเงาที่อ่านมาจากเว็บ ไม่ได้ตั้งเลขใหม่ลอย ๆ
 *
...
/**
 * ช่วงขนาดที่เปิดขายเล็กกว่าที่เว็บบอกว่าทำได้ = ยอมได้ (ร้านเลือกเองว่าจะเปิดช่วงไหน)
...
/** เปิดช่วงไม่ถึงขนาดใหญ่สุดที่ร้านทำได้ = ต้องบอกลูกค้าว่างานใหญ่กว่านั้นให้ทักแอดมิน */
const SMALLER_THAN_MAX = BIGGEST.mm < MAX_W * 10;
/** มีช่วงเดียว = ไม่มีอะไรให้ "เลือก" ข้อความจึงต้องเป็นการบอกสเปก ไม่ใช่ชวนเปรียบเทียบช่วง */
const ONE_SIZE = SIZES.length === 1;
...
console.log(`   ขนาดไม่เกิน ${MAX_W}×${MAX_H} ซม. · เคลือบเงา/ด้าน +฿${COAT_FEE}/${SHEET_UNIT}/ด้าน · เคลือบพิเศษ +฿${SPECIAL_FEE}/${SHEET_UNIT}/ด้าน`);
...
// การ์ดเทียบขนาดมีเมื่อมีช่วงขนาดตั้งแต่ 2 ช่วงขึ้นไปเท่านั้น (ดู sizeCards ในไฟล์ art)
const ART_FILES = [
  ...SIZES.map((s) => s.key),
  ...(ONE_SIZE ? [] : ["size-compare"]),
  "coat-none",
  "coat-gloss",
  "coat-matte",
  "coat-special",
...
d.description =
  ...
  `ไม่มีขั้นต่ำในการสั่งผลิต · เพิ่มเคลือบเงา เคลือบด้าน หรือเคลือบพิเศษ (กลิตเตอร์ · ทราย · โฮโลแกรม) ได้ทั้งสองด้าน`;
...
d.highlights = [
  ...
  `เคลือบเงา / เคลือบด้าน ฿${COAT_FEE} · เคลือบพิเศษ ฿${SPECIAL_FEE} — คิดต่อ${SHEET_UNIT} ต่อด้าน ไม่ใช่ต่อ${UNIT}`,
...
d.options = [
  {
    ...
    label: SIZE_LABEL,
    note:
      `ไดคัทตามทรงที่ออกแบบมา ขนาดไม่เกิน ${MAX_W} × ${MAX_H} ซม. · ราคาต่อ${UNIT}เท่ากันทุกขนาดในกรอบนี้ — ` +
      (ONE_SIZE
        ? `ค่าเคลือบคิดต่อ${SHEET_UNIT} โดย 1 แผ่น A3 ตัดได้ ${BIGGEST.perSheet} ${UNIT}`
        : `ช่วงขนาดใช้คิดค่าเคลือบต่อ${SHEET_UNIT}`) +
      (SMALLER_THAN_MAX ? ` · ใหญ่กว่าช่วงในลิสต์ทักแอดมินได้เลย` : ""),
    choices: SIZES.map((s) => ({
      name: s.name,
  ...
  },
  {
    ...
    choices: [
      ...
      // ร้านยืนยันว่าเคลือบด้านคิดเท่าเคลือบเงา (หน้าเว็บเขียนไว้แค่ "เงา")
...
d.terms = [
  ...
  `ค่าเคลือบคิดเป็นค่าวัสดุ "ต่อ 1 ${SHEET_UNIT} ต่อด้าน" ไม่ใช่ต่อ${UNIT} — เคลือบเงา / เคลือบด้าน ${COAT_FEE} บาท · เคลือบพิเศษ ${SPECIAL_FEE} บาท`,
...
d.tabs = [
  {
    ...
    text: [
      ...
      "::ขนาดงาน::",
      `• ไดคัทตามทรงที่ลูกค้าออกแบบมา — ขนาดไม่เกิน ${MAX_W} × ${MAX_H} ซม.`,
      `• ราคาต่อ${UNIT}เท่ากันทุกขนาดในกรอบนี้ · ${ONE_SIZE ? "ค่าเคลือบคิดต่อ" + SHEET_UNIT : "ช่วงขนาดที่เลือกในหน้าสินค้าใช้คิดค่าเคลือบต่อ" + SHEET_UNIT}`,
      ...
      "::ราคาบวกเพิ่ม::",
      `• เคลือบเงา / เคลือบด้าน บวกเพิ่ม ${COAT_FEE} บาท ต่อ${SHEET_UNIT} ต่อด้าน`,
    ...
    // ช่วงเดียว = ไม่มีการ์ดเทียบขนาด ใช้การ์ดของช่วงนั้น (มีผังแผ่น A3 อยู่ในตัวอยู่แล้ว)
    images: [art["print-2side"], art["size-compare"] ?? art[BIGGEST.key]],
    imagePos: "bottom",
    imageSize: "md",
  },
  {
...
d.seo = {
  ...
  description:
    ...
    `ขนาดไม่เกิน ${MAX_W} × ${MAX_H} ซม. เคลือบเงา/ด้าน/กลิตเตอร์/โฮโลแกรมได้ ไม่มีขั้นต่ำในการสั่งผลิต`,
  keywords: [
    "พัดกระดาษ",
    "พัดไดคัท",
    "รับทำพัดกระดาษ",
  ...
  faqs: [
    ...
    },
    {
      ...
      a: `เคลือบเงา หรือเคลือบด้าน ${COAT_FEE} บาท · เคลือบพิเศษ (กลิตเตอร์ ทราย โฮโลแกรม) ${SPECIAL_FEE} บาท — คิดต่อ 1 ${SHEET_UNIT} ต่อด้าน ไม่ใช่ต่อ${UNIT} · 1 แผ่น A3 ตัดได้ ${sheetExample} สั่งไม่ถึงโควตาก็คิด 1 แผ่นเต็ม`,
    },
    {
      q: "คละลายได้กี่ลาย?",
