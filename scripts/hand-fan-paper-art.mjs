 ...
 *
 * ⚠️ เคยมีช่วงย่อย "ไม่เกิน 14 × 14 ซม." (6 อัน/แผ่น) กับ "ไม่เกิน 10 × 10 ซม." (8 อัน/แผ่น)
 *    แต่ร้านสั่งถอดออก 21 ส.ค. 69 — เหลือช่วงเดียวเท่าขนาดใหญ่สุดที่ร้านทำได้
 *    ผลคือค่าเคลือบคิดจากฐาน 2 อัน/แผ่น A3 เสมอ ไม่ว่าลูกค้าจะสั่งพัดเล็กแค่ไหน
 *    (เหลือรายการเดียวก็ยังต้องเป็น "กลุ่มตัวเลือก" อยู่ เพราะ sheetFee อ่าน perSheet จากกลุ่มนี้)
 */
export const SIZES = [
...
async function sizeCards() {
  ...
  /*
    การ์ดรวม: ทุกช่วงเรียงข้างกันบนเส้นฐานเดียว สเกลเดียวกัน
    มีช่วงเดียวก็ไม่ต้องทำ — "เทียบขนาด" ที่มีของอยู่ชิ้นเดียวไม่ได้บอกอะไร
    (apply.mjs เช็ค SIZES.length เหมือนกัน จะได้ไม่ไปหาไฟล์ที่ไม่มี)
  */
  ...
  await saveSvg(
    "size-compare",
    frame(`
    ${title("ช่วงขนาดพัด", `เทียบขนาดจริง — สเกลเดียวกันทั้ง ${SIZES.length} ช่วง`)}
    ${row}
...
async function coatCards() {
  ...
  await coatCard(
    ...
    ["นวลตา ไม่สะท้อนแสง ลายนิ้วมือไม่ติด", "คิดเพิ่มต่อแผ่น A3 ต่อด้านที่เคลือบ (เท่าเคลือบเงา)"]
  );
  await coatCard(
    "coat-special",
    "เคลือบพิเศษ",
