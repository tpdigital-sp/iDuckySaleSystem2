...
--
-- ⚠️ ทำไมชื่อ product_claims ไม่ใช่ claims:
--    ฐานข้อมูลมีตาราง claims ค้างอยู่จากดีไซน์เชิงสัมพันธ์รุ่นเก่าที่เลิกใช้แล้ว
--    (order_id เป็น uuid ทั้งที่ออเดอร์จริงเป็นข้อความ OD-xxxxxx-xxxx · ว่างเปล่า · ไม่มีโค้ดไหนใช้)
--    เลี่ยงชื่อชนดีกว่าลบของเก่าทิ้ง — ถ้าภายหลังยืนยันว่าลบได้ ค่อยเปลี่ยนมาใช้ชื่อ claims
...
create index if not exists product_claims_customer_created_idx
  on public.product_claims ((data->>'customerId'), created_at desc);
